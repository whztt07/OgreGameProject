...
CharRacesDB::Record CharRacesDB::getByName(std::string name)
{
	for(Iterator i=begin(); i!=end(); ++i)
	{
		if(!stricmp(name.c_str(), i->getString(Name)))
...
CreatureModelDB::Record CreatureModelDB::getByFilename(std::string fn, bool &bFind)
{
	int nCount = 0;
...
const ItemRecord& ItemDatabase::get(int id)
{
	...
	{
		int nIndex = itemLookup[id];
		if (nIndex < 0 || nIndex > items.size() - 1)
		{
			return items[0];
		}
...
SpellEffectsDB::Record SpellEffectsDB::getByName(const std::string name)
{
	for(Iterator i=begin(); i!=end(); ++i)
	{
		if( !stricmp(name.c_str(), i->getString(EffectName)) )
