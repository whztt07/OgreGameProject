...
static int TestOpenPatchedArchive(const TCHAR * szMpqName, ...)
{
    ...
    HANDLE hMpq = NULL;
    va_list argList;
    const char * szFileName = "DBFilesClient\\AreaTable.dbc";
...
*/
    ...
    if(nError == ERROR_SUCCESS)
    {
        SFileExtractFile(hMpq, szFileName, _T("E:\\AreaTable.dbc"), 0);
...
//      nError = TestSectorCompress(MPQ_SECTOR_SIZE);
    ...
    if(nError == ERROR_SUCCESS)
        nError = TestArchiveOpenAndClose(MAKE_PATH("test_3.mpq"));
