...
int RefCheckFindDepEnumProc::proc(ReferenceMaker *rm) {
	if (count == index)
	{
		rmaker = rm;
   ...
   private:
      ...
      RefResult (*pNotifyFunc)(Interval changeInt,
         RefTargetHandle hTarget, PartID& partID,
            RefMessage message, DWORD_PTR pData);
      RefTargetHandle ref0;
      DWORD_PTR pPrivateData;
   public:
      ...
      RefTargetHandle hTarget, PartID& partID,RefMessage message, 
         DWORD_PTR pData), DWORD_PTR pData);
   ...
   if (fulltree) {
     ...
     ReferenceMaker* maker = NULL;
      while ((maker = di.Next()) != NULL) {
        FormatObjectDisplay(count, maker, showaddress, buf);
...
static bool OkToDisplay(HWND hWnd, ReferenceMaker* rmaker)
{
   ...
   if (rmaker == &notifyObject)
      return false;
   else if (rmaker->ClassID() == Class_ID(0,0)) {
      TSTR name;
...
void RefWatch::BeginEditParams(Interface *ip, IUtil *iutil) {
   if (!hMain) {
      notifyObject.SetNotifyFunc(ProcessRefMessage, reinterpret_cast<DWORD_PTR>(this));
      hMain = CreateDialogParam(hInstance, MAKEINTRESOURCE(IDD_MAIN),
         IP->GetMAXHWnd(), MainDlgProc, reinterpret_cast<LPARAM>(this));
