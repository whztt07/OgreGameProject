...
void SimpleMod::SimpleModClone( SimpleMod *smodSource )
{
	...
	if ( smodSource->tmControl ) {		
		ReplaceReference(0,remap->CloneRef(smodSource->tmControl)); 
	}
	if ( smodSource->posControl ) {		
		ReplaceReference(1,remap->CloneRef(smodSource->posControl)); 
	}
	remap->Backpatch();
	remap->DeleteThis();
}

void SimpleMod::SimpleModClone( SimpleMod *smodSource, RemapDir& remap )
	{
	if ( smodSource->tmControl ) {		
		ReplaceReference(0,remap.CloneRef(smodSource->tmControl)); 
		}
...
void SimpleWSMMod::SimpleWSMModClone(SimpleWSMMod *smodSource, RemapDir& remap)
	{	
	if (smodSource->pblock) ReplaceReference(SIMPWSMMOD_PBLOCKREF,remap.CloneRef(smodSource->pblock));
	ReplaceReference(SIMPWSMMOD_OBREF,smodSource->obRef);
	ReplaceReference(SIMPWSMMOD_NODEREF,smodSource->nodeRef);
	}
...
RefTargetHandle SimpleOSMToWSMMod::Clone(RemapDir& remap)
	...
	mod->SimpleWSMModClone(this, remap);
