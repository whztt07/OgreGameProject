	...
	void LinkConstTransform::RedrawListbox(TimeValue t,  int sel)
		{
			...
			for (int i = 0; i < ct; i++){
				...
				const char* name = node ? node->GetName() : "World";

				TSTR str;
				str.printf( "%d\t%s", linkTime / GetTicksPerFrame(), name );
				...
				{
					TCHAR buf[1000];
					SendDlgItemMessage(hWnd, IDC_SCALAR_LIST, LB_GETTEXT, i, (LPARAM)buf);
					if(strcmp(buf,str.data())==0)
					{
					  //do nothing
					}
					else
...
void LinkConstTransform::SortNodes(int sel)
	...
	if(curCurSel>=0)
		pblock->GetValue(link_target_list, 0, selINode1, FOREVER, curCurSel);
	...
	if(curCurSel>=0)
		pblock->GetValue(link_target_list, 0, selINode2, FOREVER, curCurSel);
