...
class ListDummyEntry : public Control {
	public:
		Control *lc;  //chagned by mz in order to have layer controllers also use this class. no reason to be a ListControl class
		void Init(Control *l);
...
class ListControl : public IListControl, public ILockedTrackImp {		
	public:
		...
		virtual float		GetSubCtrlWeight(int in_index, TimeValue in_time) const;
		...
		BOOL CanAssignController(int subAnim) {return !GetLocked();}
...
bool ActiveOnlyFilter(Animatable* anim, Animatable* parent, int subAnimIndex, Animatable* grandParent, INode* node)
{
	...
	{
		if (subAnimIndex == ((ListControl*)parent)->GetActive() || subAnimIndex > ((ListControl*)parent)->GetListCount())
			return true;
		return false;
	}
	return true;
...
bool NoWeightsFilter(Animatable* anim, Animatable* parent, int subAnimIndex, Animatable* grandParent, INode* node)
{
	if(parent != NULL && 
		...
		parent->ClassID() == Class_ID(POINT4LIST_CONTROL_CLASS_ID, 0)))
	{
		// This is as simple as filtering out the paramer block
		// at least for now...
...
void ListCtrlNotifyStartup(void *param, NotifyInfo *info)
{
	...
	if (tb != NULL)
	{
		...
		{
			filterManager->RegisterFilter(ActiveOnlyFilter, NULL, GetString(IDS_AF_ACTIVE_ONLY_FILTER), Class_ID(0x39822380, 0x69326448));
			filterManager->RegisterFilter(NoWeightsFilter, NULL, GetString(IDS_AF_NO_WEIGHTS_FILTER), Class_ID(0x5434724d, 0x10b22061));
		}
	}
}
...
class CutRestore : public RestoreObj, public ReferenceMaker {	
	public:	
		...
		Class_ID ClassID() { return Class_ID(0x65b762b, 0xb335f61); }
...
void ListDummyEntry::Init(Control *l) {lc=l;}
...
{
	if (!conts.Count()) return TRUE;
	assert(active>=0);
	assert(active<conts.Count());
	return conts[active]->ChangeParents( t, oldP, newP, tm );
}
...
float ListControl::GetSubCtrlWeight(int in_index, TimeValue in_time) const
{
	float val = -1.0f;
	if (in_index >= 0 && in_index < names.Count() &&
		pblock && in_index < pblock->Count(pblock->IndextoID(kListCtrlWeight))) {
		Interval valid;
		pblock->GetValue(pblock->IndextoID(kListCtrlWeight), in_time, val, valid, in_index);
	}
	return val;
}
...
void ListControlDlg::PasteItem(int a)
	...
	cont->ReplaceReference(a,cont->clip);	
	if (cont->active >= a) cont->active++;	
	cont->NotifyDependents(FOREVER,0,REFMSG_CHANGE);
	cont->NotifyDependents(FOREVER,0,REFMSG_SUBANIM_STRUCTURE_CHANGED);
	theHold.Put(new RemoveItemRestore(cont,1));
...
static INT_PTR CALLBACK ListControlDlgProc(
   ...
   ListControlDlg *ld = DLGetWindowLongPtr<ListControlDlg*>(hWnd);
			// WIN64 Cleanup: Shuler
...
void ListControl::EndEditParams(IObjParam *ip, ULONG flags,Animatable *next)
	...
	//if (!next || next->ClassID()!=ClassID()) {	
		if (dlg) {
			if(dlg->ip)
				dlg->ip->DeleteRollupPage(dlg->hWnd);
			dlg        = NULL;
			}
		paramFlags = 0;
	//	}
	...
	public:
		TSTR GetLocalName(ReferenceMaker* owner, ParamID id, int tabIndex) {
			...
			if (id == kListCtrlWeight)
				...
				if (ctrl)
					name.printf(_T("%s: %s"), GetString(IDS_AF_LIST_WEIGHT), ctrl->SubAnimName(tabIndex));
				else
