...
void Jiggle::MouseCycleStarted(TimeValue t)
{
...
void Jiggle::MouseCycleCompleted(TimeValue t)
{
	if (GetCOREInterface8()->GetSpringQuickEditMode()) {
...
{	
	...
	if ( !(flags & HAS_NO_PARENT) && (!dyn_pb->Count(jig_control_node) || dyn_pb->GetINode(jig_control_node, 0, 0) == NULL || selfNode == NULL) )
	{
		...
		TSTR nodename;
		NotifyDependents(FOREVER, reinterpret_cast<ULONG_PTR>(&nodename), REFMSG_GET_NODE_NAME);
