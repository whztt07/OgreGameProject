...
class IndePosition : public LockableControl {
   public:
      Control *posX;
      Control *posY;
      Control *posZ;
      ...
      class SetValueRestore : public RestoreObj {
          ...
          bool m_preSetValue;
      public:
          SetValueRestore::SetValueRestore(IndePosition* cont, bool preSetValue) : m_cont(cont), m_preSetValue(preSetValue) 
          {
...
public:
   ...
   class SetValueRestore : public RestoreObj {
       ...
       bool m_preSetValue;
   public:
       SetValueRestore::SetValueRestore(IndePoint4* cont, bool preSetValue) : m_cont(cont), m_preSetValue(preSetValue) 
       {
           m_cont->SetAFlag(A_HELD); 
       }
       void Restore(int isUndo) {
...
void IPosDlg::EndingEdit(IndePosition *next)
   ...
   switch (cur) {
      case EDIT_X:
		  cont->posX->EndEditParams(ip,0,next? next->posX : NULL);
         break;
      case EDIT_Y:
		  cont->posY->EndEditParams(ip,0,next?  next->posY : NULL);
         break;
      case EDIT_Z:
		  cont->posZ->EndEditParams(ip,0,next ? next->posZ : NULL);
         break;
      }
   cont = NULL;
   ip   = NULL;
