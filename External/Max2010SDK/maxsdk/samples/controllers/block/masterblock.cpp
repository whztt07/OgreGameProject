...
void MasterBlockControl::SetReference(int i, RefTargetHandle rtarg) 
	...
	else DebugPrint(_T("set reference error occurred\n"));
...
{
/*DebugPrint(_T("Flags HITTRACK_SELONLY %d HITTRACK_UNSELONLY %d HITTRACK_ABORT %d \n"),
			 flags&HITTRACK_SELONLY,
			 flags&HITTRACK_UNSELONLY,
			 flags&HITTRACK_ABORTONHIT
			 );
DebugPrint(_T("Result HITCURVE_WHOLE %d HITCURVE_KEY %d\n"),
			 result&HITCURVE_WHOLE,
			 result&HITCURVE_KEY
			 );
*/
...
//				}
	...
	for (int i=ct; i < propTargetList.Count(); i++)
		{
...
*/
	...
	case WM_COMMAND:
		...
		case IDC_MATCH_CONTROL:
			...
			for (int i=0; i<mb->propTargetList.Count();i++)
				{
				char *name;
				name = *mb->propNames[i];
				SendMessage(GetDlgItem(hWnd,IDC_LIST2),
