...
void BoolCntrl::CommitValue(TimeValue t) {
	if(GetLocked()==false)
	{
	   if (TestAFlag(A_SET)) {    
		  ...
		  if (ivalid.InInterval(t)) {
			 if (keys.Count()==0) {
				 if (Animating()&&( GetCOREInterface10()->GetAutoKeyDefaultKeyOn()&& t!=0 &&!(t==GetTicksPerFrame()&&GetCOREInterface10()->GetAutoKeyDefaultKeyTime()==GetTicksPerFrame()))) {              
				   BKey k;
...
// bc->ReplaceReference(BOOL_PBLOCK_REF, remap.CloneRef(pblock));
   bc->curval     = curval;
...
BOOL StdDrawFCurveCallback::DoKey(
   ...
   if (showStat && sel) {
      ...
      _stprintf(buf,_T("%s, %.3f"),(TCHAR *)stime,dim->Convert(kval));
