...
case REFMSG_CHANGE:
	...
	if (hTarget != pShader) // && hTarget != maps)
	{
		...
		{
			IMtlEditInterface* IMtlEditor = GetMtlEditInterface ();
			MtlBase* curMtl = IMtlEditor->GetCurMtl();
			for(int i = 0; i < N_ID_CHANNELS; ++i)
			{
				if((*maps)[i].map && curMtl == (*maps)[i].map)
				{
					DiscardTexHandle(IdToUsageTab[i]);
					break;
				}
			}
		}
...
//		val = pShader->GetAmbientClr(t);
	...
	else if(_tcscmp(paramName,_T("msl_max_stdmtl2_1_selfillumination_color"))==0)
	{
		...
		{
			float f = pShader->GetSelfIllum(t);
			f*=0.3333333f;
			val.r = val.g = val.b = f;
		}
		*(D3DCOLORVALUE*)value = GetD3DColor(val);
