...
void Mix::SetupGfxMultiMapsForHW(TimeValue t, Material *mtl, MtlMakerCallback &cb)
{
	...
	else if(2 == texNum)
	{
		if(!bmi[2]){
			...
			if(bmi[0]->bmiHeader.biHeight == bmi[1]->bmiHeader.biHeight &&
				bmi[0]->bmiHeader.biWidth == bmi[1]->bmiHeader.biWidth && SameUV(sub[0], sub[1]))
			{
			...
			if(theBM->bmiHeader.biWidth == bmi[2]->bmiHeader.biWidth &&
				theBM->bmiHeader.biHeight == bmi[2]->bmiHeader.biHeight && SameUV(theTex, sub[2])){
		...
		if(SameUV(sub[0], sub[1]) && SameUV(sub[1], sub[2]) &&
			bmi[0]->bmiHeader.biHeight == bmi[1]->bmiHeader.biHeight &&
			bmi[1]->bmiHeader.biHeight == bmi[2]->bmiHeader.biHeight &&
			bmi[0]->bmiHeader.biWidth == bmi[1]->bmiHeader.biWidth &&
			bmi[1]->bmiHeader.biWidth == bmi[2]->bmiHeader.biWidth )
		{
