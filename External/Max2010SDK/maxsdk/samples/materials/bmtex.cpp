	...
	void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval &valid)    // get into v
	{
		...
		switch (id)
		{
			case bmtex_filename:
			{
				...
				static TSTR assetId;
				assetId = IAssetManager::GetInstance()->AssetIdToString(bmt->GetMap().GetId());
				v.s = assetId;
				break;
			}
...
void BMTex::SetMap(const MaxSDK::AssetManagement::AssetUser& asset, bool isUIAction)
{
	...
	if (BMMGetUniversalName(uncname,asset.GetFileName()))
			...
			// if so, limit rollout update to the changing item and update any active viewport texture
				...
				if (changing_param == bmtex_bitmap)
				{
					if (!loading) 
					{
						...
						// Update setting for compatibility with the bitmap, when it's first set
						UpdateDefaults();
						// send message so browser/navigator name gets updated
...
// Update setting for compatibility with the bitmap.
// Used only when the bitmap is first set; allow user to override settings otherwise.
void BMTex::UpdateDefaults()
{
	Bitmap* bitmap = m_theBitmap.GetBitmap();
	if( bitmap!=NULL )
		SetPremultAlpha( bitmap->PreMultipliedAlpha() );
}
