...
#ifdef LOCALIZED_APL		// naming conflicts in APL (only?) -- LOCALIZATION_TODO
	const TCHAR *	ClassName() { return GetString(IDS_SWIRL_NAME); }
#else
...
static INT_PTR CALLBACK  PanelDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
   ...
   if (msg==WM_INITDIALOG) {
      ...
      DLSetWindowLongPtr(hwndDlg, lParam);
       ...
       if ( (theDlg = DLGetWindowLongPtr<SwirlDlg *>(hwndDlg) ) == NULL )
         return FALSE; 
      }
   theDlg->isActive = 1;
...
INT_PTR SwirlDlg::PanelProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam ) {
