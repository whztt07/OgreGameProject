...
class ChCylinderObject : public SimpleObject, public IParamArray, public RealWorldMapSizeInterface {
	public:
		// Class vars
...
void ChCylFilletDlgProc::Update()
{
    if (ob == NULL) return;
...
ChCylinderObject::ChCylinderObject(BOOL loading) 
	{
	ReplaceReference(0, CreateParameterBlock(chcyldescVer0, PBLOCK_LENGTH, CURRENT_VERSION));
...
void ChCylinderObject::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
	...
	if (pmapCreate && pmapParam) {
        ...
        ChCylFilletDlgProc* dlg = static_cast<ChCylFilletDlgProc*>(pmapParam->GetUserDlgProc());
        if (dlg != NULL) {
