...
static int char_to_code(char buffer[]);
...
//DebugPrint(_T("String: [%s] %d chars\n"),string.data(), chars);
	...
	for(i = 0; i < chars; ++i) {
		...
		if ((string[i] == '\\') && (string[i+1] == 'U' || string[i+1] == 'u') && (string[i+2] == '+')){
			int unicode_code = char_to_code(&string[i]+3);
			if(unicode_code < 0){
