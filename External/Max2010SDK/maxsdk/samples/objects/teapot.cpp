...
class TeapotObject : public SimpleObject, public IParamArray, public RealWorldMapSizeInterface  {
	public:			
		// Class vars
...
class TeapotParamDlgProc : public ParamMapUserDlgProc {
	public:
...
void TeapotParamDlgProc::Update()
{
...
void TeapotObject::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
	...
	if (pmapCreate && pmapParam) {
        ...
        TeapotParamDlgProc* dlg = static_cast<TeapotParamDlgProc*>(pmapParam->GetUserDlgProc());
        if (dlg != NULL) {
	...
	if(pmapParam) {
		// A callback for the type in.
		pmapParam->SetUserDlgProc(new TeapotParamDlgProc(this));
    }
	}
