...
void PArrayParticle::SetupTargetList()		
	...
	{if ( ((Namelen!=NLEN)&& (fileok=GenNewNameLen(size,custsettings,f,filename,CURRENT_VERSION)) == TRUE)||
		((vers!=CURRENT_VERSION)&&(fileok=GenNewSaveFile(osize,size,custsettings,f,filename,CURRENT_VERSION)) == TRUE))
...
{ BOOL found=FALSE;int cnt=gl.Count(),i=0;
  while ((!found)&&(i<cnt))
    if ((found=(gl[i].fnum==num)) == FALSE) i++;
	return (found?i:-1);
}
...
{ int count,i,j,lastnum,fullcount;
  ...
  { i=0;
	...
	{	topptr[0]=f[toplist[i]].v[0];topptr[1]=f[toplist[i]].v[1];
		...
		int maxs=0;	float Mindist(0.0f);int ThreeFace=-1; 
