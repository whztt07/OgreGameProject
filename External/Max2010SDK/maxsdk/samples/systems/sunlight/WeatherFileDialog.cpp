...
{
	_tcscpy(title,GetString(IDS_WEATHER_FILE));
...
void WeatherFileDialog::GetWeatherFile(HWND hWnd)
{
	...
	// All Cache Files
	fl.Append(GetString(IDS_ALL_WEATHER_FILES));
	fl.Append(GetString(IDS_ALL_WEATHER_FILES_WILDCARD));

	fl.Append( GetString(IDS_ALL_FILES));
	fl.Append( GetString(IDS_ALL_FILES_WILDCARD));

	TSTR title = GetString(IDS_OPEN_WEATHER_FILE);
...
//	ofn.FlagsEx         = OFN_EX_NOPLACESBAR;
	...
	ofn.lpstrDefExt     = GetString(IDS_WEATHER_EXT);
...
{
	mData = data;
	if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_WEATHER_DATA_FILE),
...
static void SetText(HWND hWnd, TCHAR *text)
{
...
void WeatherFileDialog::InitDialog(HWND hDlg)
{
	...
	else
	{
		...
		SetText(GetDlgItem(hDlg,IDC_WEATHER_FILENAME), GetString(IDS_NO_WEATHER_FILE_LOADED));
		valid = FALSE;
	}
	...
	if(valid)
	{
		...
		if(mWeatherFileCache.mWeatherHeaderInfo.mLat>0)
			sprintf(lat,"(%4.3f %s, ",mWeatherFileCache.mWeatherHeaderInfo.mLat,GetString(IDS_NORTH_INITIAL));
		else
			sprintf(lat,"(%4.3f %s, ",-mWeatherFileCache.mWeatherHeaderInfo.mLat,GetString(IDS_SOUTH_INITIAL));

		if(mWeatherFileCache.mWeatherHeaderInfo.mLon>0)
			sprintf(lon,"%4.3f %s)",mWeatherFileCache.mWeatherHeaderInfo.mLon,GetString(IDS_EAST_INITIAL));
		else
			sprintf(lon,"%4.3f %s)",-mWeatherFileCache.mWeatherHeaderInfo.mLon,GetString(IDS_WEST_INITIAL));
...
//		SetText(GetDlgItem(hDlg,IDC_TIME_ZONE),getTimeZone(mWeatherFileCache.mWeatherHeaderInfo.mLon));
		...
		{
			sprintf(str,"%3.1f %s",mWeatherFileCache.mWeatherHeaderInfo.mNumRecordsPerHour,GetString(IDS_PER_HOUR));
			SetText(GetDlgItem(hDlg,IDC_DATA_PERIOD),str);
		}
		else
		{
			SetText(GetDlgItem(hDlg,IDC_DATA_PERIOD),GetString(IDS_MULTIPLE));
		}
	...
	{
		TSTR error = TSTR("");
		SetText(GetDlgItem(hDlg,IDC_TIME_ZONE),GetString(IDS_ERROR_READING_WEATHER_FILE));
	...
	setPeriod1->SetTooltip(true,GetString(IDS_CHANGE_TIME_PERIOD));
	ICustButton *setPeriod2 = GetICustButton(GetDlgItem(hDlg,IDC_SET_PERIOD2));
	setPeriod2->SetTooltip(true,GetString(IDS_CHANGE_TIME_PERIOD));
	ICustButton *setPeriod3 = GetICustButton(GetDlgItem(hDlg,IDC_SET_PERIOD3));
	setPeriod3->SetTooltip(true,GetString(IDS_CHANGE_TIME_PERIOD));
	...
	for (int i=0; i<WeatherCacheFilter::eEnd; i++)
	{
		...
		if (LoadString(hInstance, perFrameTable[i], name, MAX_PATH) != 0)
		{
	...
	//if (pBut) pBut->SetTooltip(TRUE, GetString(IDS_CLEAR_WEATHER_FILE));
...
TSTR WeatherFileDialog::GetDateString(Int3 &date)
{
	...
	static int month [] = {
		...
		TSTR dayStr = TSTR(GetString(day[dayofWeek]));
		TSTR monStr = TSTR(GetString(month[x-1]));
...
void WeatherFileDialog::SetPeriod(HWND hParent,Int3 &MDY,Int3 &HMS)
{
	...
	if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_SET_PERIOD),
...
void WeatherFileDialog::SetDaysInMonth(HWND hDlg,Int3 &MDY)
{
	...
	else modays = mdays[which][month]-mdays[which][month-1];
