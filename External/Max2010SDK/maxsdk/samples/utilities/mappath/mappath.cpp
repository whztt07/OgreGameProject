...
class ResMapEnum : public AnimEnum {
	public:	
		...
		BOOL AddDxMtl(MtlBase * mtl)
		{
			...
			int idx = SendMessage(GetDlgItem(hDialog, IDC_DEPLIST), LB_ADDSTRING, 0, (LPARAM)idxm->GetEffectFile().GetFileName().data());
			SendMessage(GetDlgItem(hDialog, IDC_DEPLIST), LB_SETITEMDATA, idx, (LPARAM)mtl);
			DataIndex::Instance()->Store(idx, DataIndex::DXMATERIAL);
...
void BitmapPathEditorDialog::SetPath(TCHAR* path, BitmapTex* map)
{
...
BOOL BitmapPathEditorDialog::FindMap(TCHAR* mapName, TCHAR* newName)
{
...
int BitmapPathEditorDialog::CopyWarningPrompt(HWND hParent, TCHAR* filename)
{
