...
TOOLINFO* AngleControl::GetToolInfo()
{
   ...
   const TCHAR* pDegreeSymbol = GetString(IDS_DEGREE_SYMBOL);
   if (NULL != pDegreeSymbol)
   {
      str += pDegreeSymbol;
   }
...
void AngleControl::add_control(Rollout *ro, HWND parent, HINSTANCE hInstance, int& current_y)
{
   ...
   else if (is_point3(val))
   {
      ...
      throw TypeError (GetString(IDS_ANGLE_RANGE_MUST_BE_A_VECTOR), val);
   ...
   if(val != &unsupplied)
   {
      if (val != n_CW && val != n_CCW)
         throw RuntimeError (GetString(IDS_ANGLE_DIR_BAD_VALUE), val);
      m_dirCCW = val != n_CW;
   }
...
void AngleCtrlInit()
{
   ...
   install_rollout_control(Name::intern("Angle"), AngleControl::create);
}
