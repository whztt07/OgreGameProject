...
DataPairClass::apply(Value** arg_list, int count,  CallContext* cc)
{
	...
	{
		vl.v1 = arg_list[0]->eval()->get_heap_ptr();
		vl.v2 = arg_list[1]->eval()->get_heap_ptr();
		vl.result = new DataPair(vl.v1, vl.v2);
	}
	else if (count == 5 && arg_list[0] ==&keyarg_marker)
	{
		vl.v1_name = arg_list[1]->eval()->get_heap_ptr();
		vl.v1 = arg_list[2]->eval()->get_heap_ptr();
		vl.v2_name = arg_list[3]->eval()->get_heap_ptr();
		vl.v2 = arg_list[4]->eval()->get_heap_ptr();
