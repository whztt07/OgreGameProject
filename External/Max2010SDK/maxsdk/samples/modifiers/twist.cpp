...
static void OutputLockID (HWND hWnd, unsigned int lockID)
{
    ...
    _stprintf (string, _T("%0x"), lockID);
    DLTextOut(hDC, 0, 0, string);
    EndPaint (hWnd, &ps);
}
...
static INT_PTR CALLBACK AuthRollupProc( HWND hDlg, UINT message, 
               WPARAM wParam, LPARAM lParam )
{
    AuthInfo *authInfo = DLGetWindowLongPtr<AuthInfo *>(hDlg);
   ...
   DLSetWindowLongPtr(hDlg, authInfo);
   return TRUE;
...
#define AUTH_LENGTH 20
    ...
    static TCHAR authString[AUTH_LENGTH];
   ...
   case IDC_AUTHORIZE:
	   ...
	   _stscanf (authString, _T("%x"), &authNum);
       if (authNum != lockID) {
//    char buffer[50];
