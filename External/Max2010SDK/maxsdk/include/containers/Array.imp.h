...
template <class T>
Array<T>::Array(size_t usedLength, const T& defaultVal, size_t growLength)
: mpArray(NULL),
  mReservedLen(0),
  mUsedLen(0),
  mGrowLen(growLength)
...
Array<T>::operator == (const Array<T>& cpr) const
{
...
//
template <class T> Array<T>&
Array<T>::append(const Array<T>& otherArray)
{
...
//
template <class T> bool
Array<T>::find(const T& value, size_t& index, size_t start) const
{
...
Array<T>::find(const T& value) const
{
	return this->findFrom(value, 0);   // search from the beginning
}

template <class T> size_t
Array<T>::findFrom(const T& value, size_t start) const
{
...
// steps.
template <class T> Array<T>&
Array<T>::setLengthUsed(size_t n, const T& defaultVal)
{
	DbgAssert(n >= 0);
	if (n > mReservedLen) {
...
//
template <class T> Array<T>&
Array<T>::setLengthReserved(size_t n)
{
	DbgAssert(n >= 0);
...
//
template <class T> bool
Array<T>::remove(const T& value, size_t start)
{
...
template <class T> 
void Array<T>::sort(CompareFnc cmp) {
...
template <class T>
size_t Array<T>::quickSortPartition(T* data, size_t first, size_t last, CompareFnc cmp)
{
...
template <class T>
void Array<T>::quickSortRecursive(T* data, size_t first, size_t last, CompareFnc cmp)
{
	if (first < last)
	{
