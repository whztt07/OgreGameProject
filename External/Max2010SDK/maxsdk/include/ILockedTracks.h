...
#ifndef __LOCKED_TRACKS_H
#define __LOCKED_TRACKS_H
#include <animtbl.h>
...
To get a pointer to this class simply call <b>static_cast<ILockedTracksMan*>(GetCOREInterface(ILOCKEDTRACKSMAN_INTERFACE ))</b>
*/
...
//! To get from an existing object, call GetLockedTrackInterface(anim) which is defined animtbl.h
...
//! To get from an existing object, call GetLockedTrackClientInterface(anim) which is defined animtbl.h
...
public:
	...
	//! This function will be called to lock or unlock this clients subanim . The implementator of this function is responsible
	...
	//! of the implementator to lock it's subanim subanims if the flag is set to also lock it's subanims.  The system itself is responsible
	...
	//! \param[in] lockSubanims If true than also lock the children of the anim that you are locking. Usually this is not needed since the 
	...
	//! \param[in] subNum The subanim index of this track.
	virtual void SetLocked(bool val, bool lockSubanims,ReferenceTarget* anim,ReferenceTarget* client, int subNum)=0;
	//! \brief Return whether or not one if it's subanm tracks are locked.
	...
	//! \brief Return whether or not one if it's subanm tracks or lockable or not.
...
This class represents the combiniation of a StdControl class with the ILockedTrackImp class.
...
BOOL CanApplyEaseMultCurves(){return !GetLocked();}
5)  When a controller is locked it cannot be replaced, so you’ll need to modify the IsReplaceable() function. A sample implementation is
BOOL IsReplaceable() {return !GetLocked();}  
