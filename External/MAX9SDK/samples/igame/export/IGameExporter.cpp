...
{
	buf = splitPath;
	buf = buf + "\\" + fileName + "_" + node->GetName() + ".xml";
...
void IGameExporter::ExportChildNodeInfo(CComPtr<IXMLDOMNode> parent, IGameNode * child)
{
	...
	AddXMLAttribute(parent,_T("Name"), FixupName (child->GetName()));
	...
	else
	{
		...
		switch(obj->GetIGameType())
		{
			...
			{
				TCHAR* fn = ((IGameXRefObject*)obj)->GetOriginalFileName();
				if (fn != NULL)
				{
...
void IGameExporter::DumpMatrix(Matrix3 tm,CComPtr<IXMLDOMNode> parent)
{
	...
	if (!exportQuaternions)
		Buf.printf ("%f %f %f %f %f %f %f", ap.k.x, ap.k.y, ap.k.z, scaleAxis.x,scaleAxis.y,scaleAxis.z, scaleAxAngle);
	else
...
void IGameExporter::DumpMesh(IGameMesh * gm,CComPtr<IXMLDOMNode> geomData)
{
	...
	for(int n=0;n<numFaces;n++)
	{
		...
		if(smgroups != NULL) {
			buf.printf(" %u",(unsigned int)f->smGrp);
		...
		if(edges != NULL) {
			buf.printf(" %d %d %d", f->edgeVis[0], f->edgeVis[1], f->edgeVis[2] );
...
void IGameExporter::DumpBezierKeys(IGameControlType Type, IGameKeyTab Keys, CComPtr<IXMLDOMNode> prsData)
{
	...
	for(int i = 0;i<Keys.Count();i++)
	{
		if(Type==IGAME_POS || Type==IGAME_POINT3)
		{
			...
			buf.printf("%d %f %f %f",Keys[i].t,k.x,k.y,k.z); 
		}
		else if (Type == IGAME_ROT)
		{
			...
			{
			AngAxis a(q);
				buf.printf ("%d %f %f %f %f",Keys[i].t, a.axis.x, a.axis.y, a.axis.z, a.angle);
			}
			else
				buf.printf ("%d %f %f %f %f",Keys[i].t, q.x, q.y, q.z, q.w);
		}
		else if ( IsFloatController(Type))
		{
			float f = Keys[i].bezierKey.fval;
			buf.printf("%d %f",Keys[i].t,f);
		}
		else{
			...
			AngAxis a(sval.q);
			if(!exportQuaternions)
				buf.printf("%d %f %f %f %f %f %f %f",Keys[i].t,sval.s.x,sval.s.y,sval.s.z,a.axis.x, a.axis.y, a.axis.z, a.angle);
			else
				buf.printf("%d %f %f %f %f %f %f %f",Keys[i].t,sval.s.x,sval.s.y,sval.s.z,sval.q.x, sval.q.y,sval.q.z, sval.q.w);
   ...
   CComPtr <IXMLDOMNode> prsChild;
	...
	for(int i = 0;i<Keys.Count();i++)
	{
		...
		else if (Type == IGAME_ROT)
		{
			...
			AngAxis a(q);
				buf.printf ("%d %f %f %f %f",Keys[i].t, a.axis.x, a.axis.y, a.axis.z, a.angle);
			} else buf.printf ("%d %f %f %f %f",Keys[i].t, q.x, q.y, q.z, q.w);
		}
		else if ( IsFloatController(Type))
		{
