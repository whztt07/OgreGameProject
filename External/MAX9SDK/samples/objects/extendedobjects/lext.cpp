...
class LextObject : public SimpleObject, public IParamArray, public RealWorldMapSizeInterface {
	public:
		// Class vars
...
LextObject::LextObject(BOOL loading) 
	{
	ReplaceReference(0, CreateParameterBlock(LextdescVer0, PBLOCK_LENGTH, CURRENT_VERSION));
...
void LextObject::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
	...
	if (pmapCreate && pmapParam) {
        ...
        LextWidthDlgProc* dlg = static_cast<LextWidthDlgProc*>(pmapParam->GetUserDlgProc());
        if (dlg != NULL) {
