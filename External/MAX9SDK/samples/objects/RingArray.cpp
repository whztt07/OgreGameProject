...
{
	if (mRingMaster.hMasterParams) {
		RingMaster::numSpin->SetValue(mPrevNodeNum, FALSE );
	}
...
{	
	if (mRingMaster.hMasterParams) {
	    RingMaster::numSpin->SetValue(mNewNodeNum, FALSE );
	}
	const_cast<RingMaster&>(mRingMaster).NotifyDependents(FOREVER, PART_OBJ, REFMSG_CHANGE);
}
...
ISpinnerControl *RingMaster::numSpin = NULL;
ISpinnerControl *RingMaster::radSpin = NULL;
ISpinnerControl *RingMaster::ampSpin = NULL;
ISpinnerControl *RingMaster::cycSpin = NULL;
ISpinnerControl *RingMaster::phsSpin = NULL;

//------------------------------------------------------------------------------
RingMaster::RingMaster(int numNodes = 0) : mNodes(NULL), mPBlock(NULL)
{
	ParamBlockDesc desc[] = {
