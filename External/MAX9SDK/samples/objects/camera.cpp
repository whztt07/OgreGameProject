...
void SimpleCamera::SetFOVType(int ft) {
	if (fovType == ft || ft < 0 || ft > 2) return;
	fovType = ft;
	if (iFovType)
...
#endif // NO_CAMERA_MULTIPASS
{
	...
	pblock = NULL;
	ReplaceReference( 0, CreateParameterBlock( descV4, CAMERA_PBLOCK_COUNT, CAMERA_VERSION ) );
...
class ParallelCamera: public GenCamera
{
  ...
  friend int ParallelCameraCreateCallBack::proc(ViewExp *vpt,int msg,int point,int flags,IPoint2 m,Matrix3& mat);
   friend INT_PTR PcamCustomPanel::DlgProc(TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam);
...
ParallelCamera::ParallelCamera(void)
{
  ...
  ReplaceReference(0,_pblock = CreateParameterBlock(_parameterBlock,PCAM_PBLOCK_LENGTH,PCAM_CURRENT_VERSION));
  assert(_pblock);
  // initial values
...
void ParallelCamera::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
{
  ...
  {
    DLSetWindowLongPtr(_hStandardPanel, this);
...
{		
	...
	if(flags&END_EDIT_REMOVEUI)
  {
    ...
    _hStandardPanel = NULL;
      ...
      DLSetWindowLongPtr(_hStandardPanel, 0);
...
RefTargetHandle ParallelCamera::Clone(RemapDir& remap)
{
   ...
   newob->ReplaceReference(0,remap.CloneRef(_pblock));   
  ...
  case WM_COMMAND:
    ...
    default:
      ...
      break;
		...
		if(buttonMsg)
		{
         pCam = DLGetWindowLongPtr<ParallelCamera*>(hWnd); // retrieve current
...
INT_PTR PcamCustomPanel::DlgProc(TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
  ICustButton *btn = NULL;
  HIMAGELIST hList;
  HBITMAP	hBMP;
