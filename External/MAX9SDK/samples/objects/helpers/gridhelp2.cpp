...
INT_PTR VIZGridDlgProc::DlgProc( TimeValue t, IParamMap *map, HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
	{
	switch ( message ) {
		case WM_INITDIALOG:
...
VIZGridHelpObject::VIZGridHelpObject() : ConstObject() 
	{
	pblock = NULL;
	ReplaceReference(0, CreateParameterBlock(descVer, PBLOCK_LENGTH, CURRENT_VERSION));
	assert(pblock);
