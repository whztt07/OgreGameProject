...
class CylinderObject : public GenCylinder, public IParamArray, public RealWorldMapSizeInterface  {
	public:
		// Class vars
...
BOOL BaseParamDlgProc::GetRWSState()
{
...
INT_PTR BaseParamDlgProc::DlgProc(
		TimeValue t,IParamMap *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
    thishWnd=hWnd;
	switch (msg) {
...
CylinderObject::CylinderObject(BOOL loading) 
	{
...
void CylinderObject::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
	...
	if (pmapCreate && pmapParam) {
        ...
        BaseParamDlgProc* dlg = static_cast<BaseParamDlgProc*>(pmapParam->GetUserDlgProc());
        if (dlg != NULL) {
