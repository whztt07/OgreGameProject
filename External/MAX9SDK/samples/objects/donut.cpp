...
class DonutObject: public SimpleSpline, public IParamArray {			   
	...
	public:
		...
		TCHAR *GetObjectName() { if(mPipe) return GetString(IDS_PIPE_CLASSNAME); else return GetString(IDS_TH_DONUT); }
		void InitNodeName(TSTR& s) { if(mPipe) s = GetString(IDS_PIPE_CLASSNAME); else s = GetString(IDS_TH_DONUT); }		
		Class_ID ClassID();
		void GetClassName(TSTR& s) { if(mPipe) s = GetString(IDS_PIPE_CLASSNAME); else s = TSTR(GetString(IDS_TH_DONUT_CLASS)); }
...
TSTR DonutObject::GetParameterName(int pbIndex) 
	...
	switch (pbIndex) {
		...
		case PB_RADIUS2:
			if(mPipe)
				return TSTR(GetString(IDS_THICKNESS));
			else
