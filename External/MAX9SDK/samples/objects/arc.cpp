...
class ArcObject: public SimpleSpline, public IParamArray {			   
	...
	public:
		...
		TCHAR *GetObjectName();
		void InitNodeName(TSTR& s) { s = TSTR(GetObjectName()); }
		Class_ID ClassID();
...
TCHAR *ArcObject::GetObjectName() 
{ 
	switch(mArcType)
	{
		case QUARTERROUND: return GetString(IDS_TH_QUARTERROUND);
		case HALFROUND: return GetString(IDS_TH_HALFROUND );
		default:return GetString(IDS_TH_ARC);
	}
}

void ArcObject::GetClassName(TSTR& s)
{
	switch(mArcType)
	{
		case QUARTERROUND: s = TSTR(GetString(IDS_TH_QUARTERROUND_CLASS));
		case HALFROUND: s = TSTR(GetString(IDS_TH_HALFROUND_CLASS ));
		default: s= TSTR(GetString(IDS_TH_ARC_CLASS));
	}
}
