/**********************************************************************
 *<
	FILE:			PartAsm_GlobalFunctions.cpp
...
{ return &TheParticleChannelMultDesc; }

ClassDesc* GetPAOperatorDisplayDesc() 
{ return &ThePAOperatorDisplayDesc; }

ClassDesc* GetPAOperatorValueDesc() 
{ return &ThePAOperatorValueDesc; }

ClassDesc* GetPAOperatorCopyDesc() 
{ return &ThePAOperatorCopyDesc; }

ClassDesc* GetPAOperatorFunctionDesc() 
{ return &ThePAOperatorFunctionDesc; }

ClassDesc* GetPAOperatorRandomDesc() 
{ return &ThePAOperatorRandomDesc; }

ClassDesc* GetPAOperatorConditionDesc() 
{ return &ThePAOperatorConditionDesc; }

ClassDesc* GetPAOperatorObjectDesc() 
{ return &ThePAOperatorObjectDesc; }

ClassDesc* GetPATestDesc() 
{ return &ThePATestDesc; }
...
TCHAR* FormatUniverseVectorValue(Point3 p)
{
	...
	TSTR compon[3];
	for(int i=0; i<3; i++) {
		resVal = FormatUniverseValue(p[i]);
		compon[i] = (resVal[0] == '\0') ? _T("overflow") : resVal;
	}
	sprintf(res, "[%s, %s, %s]", compon[0], compon[1], compon[2]);
	return res;
}
...
TCHAR* FormatFloatValue(float f, int decimalPrecision)
{
	...
	if (decimalPrecision <= 0) {
		int i = (f >= 0.0f) ? int(floor(f+0.5f)) : -int(floor(-f+0.5f));
		sprintf(res, "%d", i);
	} else {
		TCHAR formatString[16];
		sprintf(formatString,"%%.%df",decimalPrecision);
		sprintf(res,formatString,f);
		int len = 0;
		for(int i=0; i<64; i++) if (res[i] == '\0') { len = i; break; }
...
TCHAR* FormatFloatVectorValue(Point3 p, int decimalPrecision)
{
	...
	TSTR compon[3];
	for(int i=0; i<3; i++) {
		resVal = FormatFloatValue(p[i], decimalPrecision);
		compon[i] = (resVal[0] == '\0') ? _T("overflow") : resVal;
	}
	sprintf(res, "[%s, %s, %s]", compon[0], compon[1], compon[2]);
	return res;
...
// generate random point in the volume of sphere with radius 1
...
Point3 TransformLocalFaceIntoLocalMeshCoord(Mesh* mesh, Point3 coord, int faceIndex)
{
	...
	if (faceIndex >= mesh->getNumFaces()) { // vertex based point
		...
		return (mesh->verts[vertexIndex] + coord.z * mesh->getNormal(vertexIndex));
	}
...
// finds closest vertex on mesh.
// returns true if the poing was found
bool ClosestVertexOnMesh(const Point3& toPoint, Mesh* mesh, Point3& worldLocation, int& vertexIndex, float& dist2)
{
	if (mesh == NULL) return false;
	int numV = mesh->getNumVerts();
	if (numV == 0) return false;

	worldLocation = mesh->verts[0];
	vertexIndex = 0;
	Point3 dif = toPoint - worldLocation;
	dist2 = DotProd(dif, dif);
	...
	for(int i=0; i<numV; i++) {
		dif = toPoint - mesh->verts[i];
		curDistance2 = DotProd(dif, dif);
		if (curDistance2 < dist2) {
			dist2 = curDistance2;
			vertexIndex = i;
			worldLocation = mesh->verts[i];
		}
	}
	return true;
}
...
bool PFNodeGeometry::GetClosestVertex(PreciseTimeValue t, const Point3& toPoint, Point3& worldLocation, int& vertexIndex)
{
	Matrix3* tm = GetTM(t);
	Mesh* mesh = GetMesh(t);
	if ((tm == NULL) || (mesh == NULL)) return false;
	...
	Point3 worldPoint;
	float distance2;
	bool res = ClosestVertexOnMesh(P, mesh, worldPoint, vertexIndex, distance2);
	if (res) worldLocation = worldPoint*(*tm);
	return res;
}
...
void TriangleAngles(const Point3& v0, const Point3& v1, const Point3& v2,
					float& angle0, float& angle1, float& angle2)
// calculates angles in the triangle(v0,v1,v2)
{
	Point3 a = v1 - v0;
	Point3 b = v2 - v0;
	Point3 c = v2 - v1;
	float lenA = LengthSquared(a);
	float lenB = LengthSquared(b);
	float lenC = LengthSquared(c);
	if ((lenA <= 0.0f) || (lenB <= 0.0f) || (lenC <= 0.0f)) { 
		// degenerate case
		angle0 = angle1 = angle2 = 0.0f; return;
	}

	float sq = LengthSquared(CrossProd(a,b));
	float t = sqrt(sq/(lenA*lenB));
	if (t >= 1.0f) {
		// degenerate case
		angle0 = angle1 = angle2 = 0.0f; return;
	} else {
		if (t > 0.0f) {
			angle0 = (float)asin(t);
			if (DotProd(a, b) < 0.0f) angle0 = PI - angle0;
		} else {
			// degenerate case
			angle0 = angle1 = angle2 = 0.0f; return;
		}
	...
	if (t >= 1.0f) {
		...
		bool acute = (DotProd(b, c) >= 0.0f);
		if (t > 0.0f) {
			angle2 = (float)asin(t);
			if (DotProd(a, b) < 0.0f) angle2 = PI - angle2;
		} else {
			// degenerate case
			angle0 = angle1 = angle2 = 0.0f; return;
		}
	}
	angle1 = PI - angle0 - angle2;
}

bool GetNeiboFace(AdjEdgeList* aeList, Mesh* mesh, int curFace, int mainVertex, int axVertex, DWORD smGroup,
				  int& neiboFace, int& nextAxVertex, int& mainLocalIndex)
{
	int curEdgeIndex = aeList->FindEdge( mainVertex, axVertex );
	if (curEdgeIndex < 0) return false;
	MEdge curEdge = aeList->edges[curEdgeIndex];
	neiboFace = curEdge.OtherFace(curFace);
	if (neiboFace < 0) return false;
	Face f = mesh->faces[neiboFace];
	DWORD nextSmGroup = f.getSmGroup();
	if ((smGroup & f.getSmGroup()) == 0) return false; // break in smoothness
	for(int i=0; i<3; i++) {
		if (f.v[i] == mainVertex) {
			mainLocalIndex = i;
			continue;
		}
		if (f.v[i] == axVertex) continue;
		nextAxVertex = f.v[i];
	}
	return true;
}

Point3* PFNodeGeometry::GetNormals(TimeValue time)
{
	if (node() == NULL) return NULL;
	...
	if (normals(index) == NULL) {
		Mesh* mesh = GetMesh(time);
		int i, j, numFaces = mesh->numFaces;
		if (numFaces > 0) {
			Tab<Point3> faceNormals;
			faceNormals.SetCount(3*numFaces);
			for(i=0; i<numFaces; i++) {
				Face f = mesh->faces[i];
				float angles[3];
				TriangleAngles(mesh->verts[f.v[0]],mesh->verts[f.v[1]],mesh->verts[f.v[2]],
								angles[0], angles[1], angles[2]);
				Point3 fN = Normalize(mesh->getFaceNormal(i));
				for(j=0; j<3; j++)
					faceNormals[3*i+j] = angles[j]*fN;
			}
			Point3* curNorms = new Point3[3*numFaces];
			_normals(index) = curNorms;
			AdjEdgeList* aeList = new AdjEdgeList(*mesh);
			for(int i=0; i<numFaces; i++) {
				Face face = mesh->faces[i];
				DWORD smGroup = face.getSmGroup();
				for(j=0; j<3; j++) {
					curNorms[3*i+j] = faceNormals[3*i+j];
					if (smGroup == 0) continue; // the face is not smoothed to any neibos
					// collect normals from one direction
					int mainVertex = face.v[j];
					int axVertex = face.v[(j+1)%3];
					int curFaceIndex = i;
					int neiboFace, nextAxVertex, mainLocalIndex;
					bool fullCircle = false;
					while (GetNeiboFace(aeList, mesh, curFaceIndex, mainVertex, axVertex, smGroup,
						neiboFace, nextAxVertex, mainLocalIndex)) 
					{
						if (neiboFace == i) { fullCircle = true; break; }
						curNorms[3*i+j] += faceNormals[3*neiboFace + mainLocalIndex];
						curFaceIndex = neiboFace;
						axVertex = nextAxVertex;
					}
					if (!fullCircle) {
						// get to the other side
						curFaceIndex = i;
						axVertex = face.v[(j+2)%3];
						while (GetNeiboFace(aeList, mesh, curFaceIndex, mainVertex, axVertex, smGroup,
							neiboFace, nextAxVertex, mainLocalIndex)) 
						{
							if (neiboFace == i) { DbgAssert(0); break; }
							curNorms[3*i+j] += faceNormals[3*neiboFace + mainLocalIndex];
							curFaceIndex = neiboFace;
							axVertex = nextAxVertex;
						}
			...
			for(int i=0; i<3*numFaces; i++)
				curNorms[i] = Normalize(curNorms[i]);
		}
	}
