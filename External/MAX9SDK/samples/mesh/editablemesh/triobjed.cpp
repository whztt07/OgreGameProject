...
EditTriObject::EditTriObject() {
	...
	ReplaceReference ( ET_MASTER_CONTROL_REF, NewDefaultMasterPointController());
}
...
void EditTriObject::ActivateSubobjSel (int level, XFormModes& modes) {
	...
	InvalidateTempData (PART_SELECT|PART_SUBSEL_TYPE); // OLP 0605 no need to invalidate geom and topo caches.
...
MeshVertRestore::MeshVertRestore(EditTriObject *et) {
	...
	for (int i=0; i<vdNum; i++) uvData[i] = et->GetMesh().vData[i];
}
...
void MeshVertRestore::Restore(int isUndo) {
	...
	if (isUndo) {
		...
		if (vdNum && !rvData) {
			...
			for (int i=0; i<vdNum; i++) rvData[i] = et->GetMesh().vData[i];
		}
	}
	...
	if (vdNum) {
		...
		for (int i=0; i<vdNum; i++) et->GetMesh().vData[i] = uvData[i];
	}
	et->GetMesh().ClearFlag(MESH_PARTIALCACHEINVALID);
	et->LocalDataChanged (PART_GEOM);
...
void MeshVertRestore::Redo() {
	...
	if (vdNum) {
		...
		for (int i=0; i<vdNum; i++) et->GetMesh().vData[i] = rvData[i];
	}
	et->GetMesh().InvalidateGeomCache();
	et->GetMesh().ClearFlag(MESH_PARTIALCACHEINVALID);
	et->LocalDataChanged (PART_GEOM);
...
ObjectState EditTriObject::Eval(TimeValue time) {	
#if defined(NEW_SOA)
	if (cont.Count() > 0) {
		GetTriObjDescriptor()->Execute(I_EXEC_EVAL_SOA_TIME, reinterpret_cast<ULONG_PTR>(&time));
	}
#endif
	...
	if (!geomValid.InInterval(time)) {
		...
		geomValid = FOREVER;
#if defined(NEW_SOA)
		Interval orgGeomValid = FOREVER;
		bool bSOAEnabled = true;
		GetTriObjDescriptor()->Execute(I_EXEC_GET_SOA_STATE, reinterpret_cast<ULONG_PTR>(&bSOAEnabled));
		if (!bSOAEnabled)
			orgGeomValid = geomValid;
#endif
		for (int i=0; i<cont.Count(); i++) {
			if (cont[i])  {
...
void EditTriObject::DragMove (MeshDelta & md, MeshDeltaUser *mdu) {
	...
	dragRestored = FALSE;
	// OLP Invalidate only modified vertices.
