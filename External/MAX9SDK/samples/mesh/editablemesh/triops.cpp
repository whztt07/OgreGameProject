...
static BOOL AppendSystemNodes(INode *attachNode, INodeTab &nodeTab)
{
    Tab<ReferenceTarget *> masterTab;
	INodeTab sysTab,finalNodeTab;

	for (int i=0; i<nodeTab.Count(); i++)
	{
		Control *tmCont = nodeTab[i]->GetTMController();
		ReferenceTarget *master = GetMasterController(tmCont);
		if (master)
		{
			if (reftargInTab(masterTab,master)) continue; // already got these system nodes
			masterTab.Append(1,&master); // add master to tab of processed masters
			// add entire system to final node tab
		    master->GetSystemNodes(sysTab, kSNCDelete);
...
int CreateFaceMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m) {	
	...
	case MOUSE_POINT:
		...
		if (HitTestVerts(m, vpt, nv)) {
			...
			MaxAssert (hr);
         int j;
			for (j=0; j<pt; j++) if (vts[j] == nv) break;
			if (j<pt) done=TRUE;
			else {
				vts.Append (1, &nv, 20);
				if (pt==0) mfirst = m;
...
void AttachHitByName::proc(INodeTab &nodeTab) {
	...
	if (!AppendSystemNodes(nodes[0],nodeTab)) {
		TSTR buf1 = GetString(IDS_SA_NO_SYS_ATTACH);
		TSTR buf2 = GetString(IDS_SCA_BASE_MESH);
		MessageBox(eo->ip->GetMAXHWnd(),buf1,buf2,MB_OK|MB_ICONINFORMATION);
		ret = FALSE;
	}
