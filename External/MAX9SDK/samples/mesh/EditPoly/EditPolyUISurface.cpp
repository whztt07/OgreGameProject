...
static SmoothingGroupUIHandler theSmoothingGroupUIHandler;
...
void SmoothingGroupUIHandler::SetSmoothing (HWND hWnd, int buttonID, EPolyMod *pMod)
{
	...
	if (set) {
		...
		if (!startFresh && (smoothers = pBlock->GetInt (epm_smooth_group_set))) {
...
static INT_PTR CALLBACK SelectBySmoothDlgProc (HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
		...
		theSmoothingGroupUIHandler.UpdateSmoothButtons (hWnd, selectBySmoothGroups, 0, param[0]);
...
static MaterialUIHandler theMaterialUIHandler;
...
INT_PTR SurfaceDlgProc::DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
	...
	case WM_INITDIALOG:
		...
		theMaterialUIHandler.SetupMtlSubNameCombo (hWnd, mpMod);           
		
		theSmoothingGroupUIHandler.InitializeButtons (hWnd);
	...
	case WM_PAINT:
		...
		theSmoothingGroupUIHandler.UpdateSmoothButtons (hWnd, mpMod);
		if (!theSmoothingGroupUIHandler.UIValid()) uiValid = NEVER;

		// Display the correct material index:
		theMaterialUIHandler.UpdateCurrentMaterial (hWnd, mpMod, t, uiValid);
	...
	case WM_COMMAND:
		...
		if (buttonID>=IDC_SMOOTH_GRP1 && buttonID<=IDC_SMOOTH_GRP32) {
			theSmoothingGroupUIHandler.SetSmoothing (hWnd, buttonID, mpMod);
			break;
		}
		...
		case IDC_MTLID_NAMES_COMBO:
			...
			case CBN_SELENDOK:
				theMaterialUIHandler.SelectByName (hWnd, mpMod);
				break;                                                    
			}
			break;

		case IDC_SELECTBYSMOOTH:
			if (theSmoothingGroupUIHandler.GetSelectBySmoothParams (mpMod))
				mpMod->EpModButtonOp (ep_op_select_by_smooth);
			break;

		case IDC_SMOOTH_CLEAR:
			theSmoothingGroupUIHandler.ClearAll (mpMod);
			break;
