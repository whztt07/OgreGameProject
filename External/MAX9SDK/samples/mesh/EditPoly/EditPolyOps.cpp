...
public:
	PolyOpMeshSmooth () : mSmoothness(0.0f), mSepBySmooth(0), mSepByMat(0) { }
...
public:
	PolyOpTessellate () : mTessTension(0.0f), mTessType(0) { }
...
public:
	PolyOpChamferVertex () : mAmount(0.0f), mOpen(false) { }
...
bool PolyOpChamferVertex::Do (MNMesh & mesh, LocalPolyOpData *pOpData)
{
	...
	{
		l_ret = l_meshToChamfer->ChamferVertices (l_flag, &chamData,mOpen);
	}
...
public:
	PolyOpWeldEdge () : mThreshold(0.0f) { }
...
public:
	PolyOpChamferEdge () : mAmount(0.0f), mOpen(false) { }
...
bool PolyOpChamferEdge::Do (MNMesh & mesh, LocalPolyOpData *pOpData)
{
	...
	if (l_meshToChamfer != NULL )
	{
		l_ret = l_meshToChamfer->ChamferEdges (l_flag, pMeshChamData,mOpen);
	}
...
public:
	PolyOpInsetFace () : mAmount(0.0f), mType(0) { }
...
public:
	PolyOpOutlineFace () : mAmount(0.0f) { }
...
public:
	PolyOpHingeFromEdge () : mSegments(1), mAngle(0.0f), mBase(0,0,0), mDir(0,0,0) { }
...
public:
	...
	~PolyOpExtrudeAlongSpline() { ClearSpline(); }
	void ClearSpline ();
...
bool PolyOpBridgeEdge::Do (MNMesh & mesh, LocalPolyOpData *pOpdata) {
	...
	if ( l_meshToBridge != NULL )
	{
		...
		if (mUseSelected) 
			ret = l_meshToBridge->BridgeSelectedEdges ( MN_EDITPOLY_OP_SELECT,mThresh, mSegments,mAdjacent, mReverseTriangles);
		else 
		{
...
void EditPolyMod::EpModLocalDataChanged (DWORD parts) {
	...
	if (parts & (PART_SELECT|PART_TOPO)) InvalidateSurfaceUI ();
...
void EditPolyMod::Transform (TimeValue t, Matrix3& partm, Matrix3 tmAxis, BOOL localOrigin, Matrix3 xfrm, int type) {
	...
	int softSel, edgeIts=1, useEdgeDist=FALSE;
	...
	for (int nd=0; nd<mcList.Count(); nd++) {
		...
		if ( (commandMode->ID() == CID_SUBOBJROTATE) ||
