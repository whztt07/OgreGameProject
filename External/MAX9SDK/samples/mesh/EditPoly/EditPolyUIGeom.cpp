...
void ConstraintUIHandler::Initialize (HWND hWnd, EPolyMod *pMod) {
	...
	HWND hDroplist = GetDlgItem (hWnd, IDC_CONSTRAINT_LIST);
	if (!hDroplist) return;
...
void ConstraintUIHandler::Set (HWND hWnd, EPolyMod *pMod) {
	if (!pMod) return;
	if (!pMod->getParamBlock()) return;
	HWND hDroplist = GetDlgItem (hWnd, IDC_CONSTRAINT_LIST);
	if (!hDroplist) return;

	int type = SendMessage (hDroplist, CB_GETCURSEL, 0, 0);
...
static PreserveMapsUIHandler thePreserveMapsUIHandler;
...
INT_PTR EditPolyGeomDlgProc::DlgProc (TimeValue t, IParamMap2 *pmap, HWND hWnd,
	...
	case WM_COMMAND:
		...
		case IDC_SETTINGS_PRESERVE_MAPS:
			thePreserveMapsUIHandler.SetEPoly (mpMod);
			thePreserveMapsUIHandler.DoDialog ();
			break;
