...
int EditPolyMod::EpModGetCommandMode () {
	...
	if (currentMode == chamferMode) {
		if (meshSelLevel[selLevel] == MNM_SL_EDGE) return ep_mode_chamfer_vertex;
		else return ep_mode_chamfer_edge;
	}
...
void EditPolyMod::EpModEnterCommandMode (int mode) {
	...
	case ep_mode_create_face:
		...
		if ((selLevel < EPM_SL_FACE) && (selLevel != EPM_SL_OBJECT)) ip->SetSubObjectLevel (EPM_SL_FACE);
...
void EditPolyCreateFaceCMode::Display (GraphicsWindow *gw) {
	if (proc.vts.Count() < 2) return;
	MNMesh *pMesh = mpEditPoly->EpModGetMesh();
	if (!pMesh) return;

	Tab<Point3> rverts;
	rverts.SetCount (proc.vts.Count()+1);
	for (int j=0; j<proc.vts.Count(); j++) rverts[j] = pMesh->P(proc.vts[j]);
	gw->setColor(LINE_COLOR,GetUIColor(COLOR_SUBSELECTION));
	gw->polyline (proc.vts.Count(), rverts.Addr(0), NULL, NULL, FALSE, NULL);
}

void EditPolyCreateFaceMouseProc::DrawCreatingFace (HWND hWnd) {
	...
	if (mpts.Count()>1) LineTo (hdc, mpts[mpts.Count()-1].x, mpts[mpts.Count()-1].y);

	DeleteObject(SelectObject(hdc,GetStockObject(BLACK_PEN)));
	ReleaseDC(hWnd, hdc);
}

HitRecord *EditPolyCreateFaceMouseProc::HitTestVerts (IPoint2 m, ViewExp *vpt) {
	vpt->ClearSubObjHitList();

	mpEPoly->SetHitLevelOverride (SUBHIT_MNVERTS|SUBHIT_OPENONLY);
	ip->SubObHitTest(ip->GetTime(),HITTYPE_POINT,0, 0, &m, vpt);
	mpEPoly->ClearHitLevelOverride ();
	if (!vpt->NumSubObjHits()) return NULL;
	...
	for (HitRecord *hr = hitLog.First(); hr; hr = hr->Next())
	{
		if (mpts.Count()>0)
		{
			// Check that this hit is on the right mesh.
			if (hr->nodeRef != mpEPoly->EpModGetPrimaryNode()) continue;
		}
		if ((bestHit == NULL) || (bestHit->distance>hr->distance)) bestHit = hr;
	}
...
int EditPolyCreateFaceMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m) {
	if (!hCurCreateVert) hCurCreateVert = LoadCursor(hInstance,MAKEINTRESOURCE(IDC_ADDVERTCUR));
	...
	case MOUSE_POINT:
		...
		{
			// No point hit, and no point created.
			// If this is the first click, we return false to indicate we should start over.
			// If this is a later click, we ignore it and return true.
			ip->ReleaseViewport (vpt);
			return point>0;
		}

		// If this click is supposed to start a face, we should figure out which node it goes on.
		if (mpts.Count() == 0)
		{
			if (hr) mpEPoly->EpModSetPrimaryNode (hr->nodeRef);
			else mpEPoly->EpModSetPrimaryNode (NULL);	// chooses one primary from the available nodes.
		}

		if (hr) {
			int nv = hr->hitInfo;

			// If we've hit this vertex before, signal that we're done.
         int j;
			for (j=0; j<mpts.Count(); j++) if (vts[j] == nv) break;
			if (j<mpts.Count()) done=TRUE;
			else {
				// Otherwise, add it to our list.  (No undo/redo here, since nothing's been created.)
				vts.Append (1, &nv, 20);
				mpts.Append (1, &m, 20);
				mpEPoly->EpModLocalDataChanged (PART_DISPLAY);
			...
			theHold.Begin ();
			int nv = mpEPoly->EpModCreateVertex (newpt);
			theHold.Accept (GetString (IDS_CREATE_VERTEX));
		...
		if (done) {
			theHold.Begin ();
			if (mpEPoly->EpModCreateFace (&vts)<0)
			{
				// Error in creation.
				theHold.Cancel ();
	...
	case MOUSE_MOVE:
		...
		if (mpts.Count()) DrawCreatingFace (hwnd);
		if (HitTestVerts(m,vpt)) {
...
void EditPolyCreateFaceMouseProc::Backspace () {
	mpts.Delete (mpts.Count()-1, 1);
	vts.Delete (vts.Count()-1, 1);
}
...
void EditPolyShapePickMode::EnterMode(IObjParam *ip) {
	...
	if (hWnd && (but = GetICustButton (GetDlgItem (hWnd, IDC_EXTRUDE_PICK_SPLINE)))) {
		but->SetCheck(true);
		ReleaseICustButton(but);
		but = NULL;
	}
	...
	if (hWnd && (but = GetICustButton (GetDlgItem (hWnd, IDC_EXTRUDE_ALONG_SPLINE)))) {
		but->SetCheck(true);
		ReleaseICustButton(but);
		but = NULL;
	}
...
int EditPolyBridgeMouseProc::proc (HWND hwnd, int msg, int point, int flags, IPoint2 m) {
	...
	case MOUSE_FREEMOVE:
		vpt = ip->GetViewport(hwnd);
		vpt->SnapPreview (m, m, NULL, SNAP_FORCE_3D_RESULT);
		if (hr=HitTest(m,vpt)) SetCursor(ip->GetSysCursor(SYSCUR_SELECT));
		else SetCursor(LoadCursor(NULL,IDC_ARROW));
		if (vpt) ip->ReleaseViewport(vpt);
		DrawDiag (hwnd, lastm);
		DrawDiag (hwnd, m);
