...
static int MapButtons[6] = {0,IDC_UNWRAP_PLANAR,IDC_UNWRAP_CYLINDRICAL,IDC_UNWRAP_SPHERICAL,IDC_UNWRAP_BOX,IDC_UNWRAP_PELT2};
...
//5.1.01
class UnwrapMod : public IUnwrapMod, TimeChangeCallback,IUnwrapMod2,IUnwrapMod3,IUnwrapMod4,IUnwrapMod5{	
	public:
		float falloffStr;
		BOOL forceUpdate;
...
//PELT
		...
		void PlugControllers(int index);
		void PlugControllers(BitArray whichVerts);
...
//				PeltDialog peltDialog;
				...
				HWND hMapParams;
				void BuildVisibleFaces(GraphicsWindow *gw, ModContext *mc, BitArray &sel);
				void HitGeomEdgeData(Tab<UVWHitData> &hitEdges,GraphicsWindow *gw,  HitRegion hr);
				void HitGeomVertData(Tab<UVWHitData> &hitVerts,GraphicsWindow *gw,  HitRegion hr);
				...
				void GetOpenEdges(Tab<int> &openEdges, Tab<int> &results);

				void fnSelectOverlap();
				void SelectOverlap(BOOL limitSel);
				...
				BOOL fnIsMesh() { return isMesh; }

				Point3 SnapPoint(Point3 p, int snapIndex);

			private:
//VSNAP
				BitArray edgesConnectedToSnapvert;
//				BOOL vSnap,eSnap;
				Tab<int> vertexSnapBuffer;
				Tab<int> edgeSnapBuffer;


				BOOL showCounter;
				
//				Tab<EdgeBondage> dspringEdges;
...
//				BitArray dverts;
				...
				void DrawGizmo(TimeValue t, INode* inode,ModContext *mc, GraphicsWindow *gw);
				...
				BOOL UpdateGeoFaceData(Mesh *msh);
				BOOL UpdateGeoFaceData(MNMesh *msh);
				BOOL UpdateGeoFaceData(PatchMesh *msh);
