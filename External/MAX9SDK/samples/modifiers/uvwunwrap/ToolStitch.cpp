...
//build my connection list first
   ...
   BitArray initialVertexSelection = vsel;
...
//this builds a list of verts connected to the selection
   ...
   if (vsel.NumberSet() == 1)
   {
      ...
      BitArray connectedVerts;
      connectedVerts.SetSize(vsel.GetSize());
      connectedVerts.ClearAll();


      for (int i = 0; i < TVMaps.ePtrList.Count(); i++)
      {
      //see if one edge vert equals out
         int a,b;
         a = TVMaps.ePtrList[i]->a;
         b = TVMaps.ePtrList[i]->b;
         
         if (a == vIndex) 
            connectedVerts.Set(b);
         if (b == vIndex) 
            connectedVerts.Set(a);
      }

      int clusterId = vertexClusterList[vIndex];
      for (int i = 0; i < TVMaps.ePtrList.Count(); i++)
      {        
         int a,b;
         a = TVMaps.ePtrList[i]->a;
         ...
         int potentialVertexOp = -1;
         if ( vertexClusterList[a] == clusterId)
         {
            potentialVertex = a;
            potentialVertexOp = b;
         }
         if ( vertexClusterList[b] == clusterId)
         {
            potentialVertex = b;
            potentialVertexOp = a;
         }
         if ((potentialVertex != -1) && (potentialVertex != vIndex))
         {
            if (connectedVerts[potentialVertexOp])
            {
               oppositeVerts.Set(potentialVertex);
               i = TVMaps.ePtrList.Count();
            }
         }
...
//build a line from selection of element 1
            ...
            Point3 targetStart = TVMaps.v[targetPoints[0]].p;
            Point3 targetEnd = TVMaps.v[targetPoints[0]].p;
            Point3 sourceStart = TVMaps.v[sourcePoints[0]].p;
            Point3 sourceEnd = TVMaps.v[sourcePoints[0]].p;
...
//new scale
            if (bScale)
               ...
               for (int i = 0; i < TVMaps.v.Count(); i++)
                  ...
                  if (vsel[i])
                     {
                        Point3 vec = (TVMaps.v[i].p - centerSource) * scale;
                        vec = centerSource + vec;
                        TVMaps.v[i].p = vec + ((TVMaps.v[i].p-vec) * fBias);
                        if (TVMaps.cont[i]) TVMaps.cont[i]->SetValue(0,&TVMaps.v[i].p);
