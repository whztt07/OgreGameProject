...
void UnwrapMod::CopySelectionPatch(ObjectState *os, ModContext &mc, int CurrentChannel, TimeValue t)
	...
	if ( (ip && (ip->GetSubObjectLevel() > 0) ))
		{
		pobj->patch.mUseSoftSelections = FALSE;
...
//6-29--99 watje
			...
			if (ip->GetSubObjectLevel() == 3)
				pobj->patch.SetDispFlag(DISP_SELPATCHES);
			}
		else
		{
			if (ip->GetSubObjectLevel() == 3)
...
void UnwrapMod::PatchUpdateGData(PatchMesh *patch, BitArray faceSel)
	...
	//see if we need to updata any thing
	if (gfaces.Count() == faceSel.NumberSet())
	{
		BOOL noChange = TRUE;
		BitArray oldFSel;
		oldFSel.SetSize(faceSel.GetSize());
		oldFSel.ClearAll();
		for (int i = 0; i < gfaces.Count(); i++)
		{
			if (gfaces[i] != NULL)
			{
				int findex = gfaces[i]->FaceIndex;
				oldFSel.Set(findex,TRUE);
			}
		}
		if (oldFSel == faceSel) return;
	}

	int vct = patch->numVecs+patch->numVerts;
	gverts.d.SetCount(vct);
	gverts.sel.SetSize(vct,1);
	gverts.sel.ClearAll();
...
//build new tv faces
	...
	for (int i =0; i < faceSel.NumberSet(); i++) gfaces[i] = NULL;
