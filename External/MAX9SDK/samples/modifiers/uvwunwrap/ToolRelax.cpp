...

{
   ...
   if (TVSubObjectMode == 1)
      GetVertSelFromEdge(vselTemp);
   else if (TVSubObjectMode == 2)
      GetVertSelFromFace(vselTemp);

   if (vselTemp.NumberSet() == 0) return;
...
*/ 
   ...
   if (peltData.peltDialog.hWnd)
      peltData.RelaxRig(iter,relax,boundary,this);
}
