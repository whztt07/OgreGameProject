...
//	void PeltData(BitArray fsel, Mesh *patch);
	...
	int HitTestPointToPointMode(UnwrapMod *mod, ViewExp *vpt,GraphicsWindow *gw,IPoint2 *p,HitRegion hr,INode* inode,ModContext* mc);
	int HitTestEdgeMode(UnwrapMod *mod, ViewExp *vpt,GraphicsWindow *gw,IPoint2 *p,HitRegion hr,INode* inode,ModContext* mc,int flags, int type);
	...
	void NukeRig(UnwrapMod *mod );

	void SetupSprings(UnwrapMod *mod);

	void ValidateSeams(UnwrapMod *mod,BitArray &newSeams);
...
public:
	...
	UnwrapPeltSeamRestore(UnwrapMod *m, Point3 *anchorPoint=NULL);
