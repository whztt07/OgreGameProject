...
//get first open vertex
   ...
   for (int i =0; i < mod->esel.GetSize(); i++)
      {
      if (mod->esel[i] )
         {
...
//convert points from screen to UVW Space
      ...
      pointList[0] = new Point3(mod->SnapPoint(p,indexList[0]));

      p.x = ((float)(m.x-center.x))/xzoom;
      p.y = -((float)(m.y-center.y))/yzoom;
      p.z = 0.0f;
      pointList[1] =  new Point3(mod->SnapPoint(p,indexList[indexList.Count()-1]));
...
//convert points from screen to UVW Space
      ...
      for (int i = 0; i < pointList.Count(); i++)
         ...
         pointList[i] = new Point3(mod->SnapPoint(p,-1));
         }
...
//apply the sketch
      ...
      snapCenter.z = 0.0f;
      snapCenter = mod->SnapPoint(snapCenter,-1);
      ...
      snapEdge.z = 0.0f;
      snapEdge = mod->SnapPoint(snapEdge,-1);
