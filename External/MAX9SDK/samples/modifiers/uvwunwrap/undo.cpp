...
TVertJustPosRestore::TVertJustPosRestore(UnwrapMod *m, BOOL update) 
	{
	mod = m;
	undo.SetCount(mod->TVMaps.v.Count());
	for (int i = 0; i < undo.Count(); i++)
		undo[i] = mod->TVMaps.v[i].p;
//	undo = mod->TVMaps.v;
	uvsel   = mod->vsel;
	uesel   = mod->esel;
	ufsel   = mod->fsel;
...
void TVertJustPosRestore::Restore(int isUndo) 
	{
	if (isUndo) 
		{
		redo.SetCount(mod->TVMaps.v.Count());
		for (int i = 0; i < redo.Count(); i++)
			redo[i] = mod->TVMaps.v[i].p;
...
void TVertJustPosRestore::Redo() 
	{
//	mod->TVMaps.v = redo;
	for (int i = 0; i < redo.Count(); i++)
		mod->TVMaps.v[i].p = redo[i];
...
void TVertJustPosRestore::EndHold() 
	{
	updateView = TRUE;
	mod->ClearAFlag(A_HELD);
	}
TSTR TVertJustPosRestore::Description() 
	{
...
TVertAndTFaceRestore::TVertAndTFaceRestore(UnwrapMod *m,bool hide) 
	{
...
void TVertAndTFaceRestore::Restore(int isUndo) 
	...
	if (mod->TVMaps.v.Count() != mod->TVMaps.cont.Count())
	{
...
void TVertAndTFaceRestore::Redo() 
	...
	if ( mod->TVMaps.cont.Count() !=  mod->TVMaps.v.Count())
	{
		...
		for (int i=start; i < mod->TVMaps.v.Count(); i++)
		{
			...
			mod->SetReference(i+11+100,NewDefaultPoint3Controller());			
		}
