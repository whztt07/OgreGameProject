		...
		if ( i < faceSelection->GetSize())
		{
			if ((*faceSelection)[i])
			{
				...
				for (int j = 0; j < ct; j++)
				{
					...
					plistUVW[j] = TVMaps.v[index].p;
					bounds += plistUVW[j];
					
					index = TVMaps.f[i]->v[j];
					plistGeom[j] = TVMaps.geomPoints[index];
				}
				if (ct == 3)
				{
...
void UnwrapMod::fnSetSeamSelection(BitArray *sel)
{
	...
	{
		if ((i < peltData.seamEdges.GetSize()) && ((*sel)[i]))
...
void UnwrapMod::ApplyQMap()
{
	...
	//center
	//align
	fnAlignAndFit(GetQMapAlign());
	//fix up scale
	Matrix3 *tm =  fnGetGizmoTM();
	Point3 x = tm->GetRow(0);
	Point3 y = tm->GetRow(1);
	if (Length(x) < Length(y))
	{
		x = Normalize(x) * Length(y);
		tm->SetRow(0,x);
	}
	else
	{
		y = Normalize(y) * Length(x);
		tm->SetRow(1,y);
	}
	fnSetGizmoTM(*tm);
	ApplyGizmo();
...
void UnwrapMod::fnSetNormalizeMap(BOOL normalize)
{
	...
	CheckDlgButton(hSelRollup,IDC_NORMALIZEMAP_CHECK2,normalizeMap);
