...
float SelMod::DistFromVolume(Point3 pt, Matrix3 &tm, Matrix3 & ctm, Box3 box)
	...
	for (int i=0; i<3; i++) scale[i] = Length (ctm.GetRow (i));
...
void SelMod::SelectVertices (Mesh &mesh, Matrix3 &tm, Matrix3 & ctm, Box3 &box, TimeValue t) {
	...
	if ((vol == SEL_MATID) || (vol == SEL_SMG)) {
		...
		case SEL_ADD:
			...
			if (vsw) {
				for (int i=0; i<mesh.numVerts; i++) if ((*selVerts)[i]) vsw[i] = 1.0f;
			}
			break;
		case SEL_SUBTRACT:
			mesh.vertSel &= ~(*selVerts);
			if (vsw) {
				for (int i=0; i<mesh.numVerts; i++) if ((*selVerts)[i]) vsw[i] = 0.0f;
			}
			break;
		}
		if (invert) {
			...
			if (vsw) {
				for (int i=0; i<mesh.numVerts; i++) vsw[i] = 1.0f-vsw[i];
			}
		}
		return;
	}
...
//		SelfNode = dep.Nodes[0];
		...
		int id;		
		SelfNode = GetNodeFromModContext(&mc,id);
...
//	HWND hWnd = pmapParam2->GetHWnd();
	...
	for (int i=1; i<7; i++) EnableWindow (GetDlgItem (hWnd, StraightEnables[i]), enable);
