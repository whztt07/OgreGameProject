...
class SplineIKControlModGeomPipelineEnumProc : public GeomPipelineEnumProc
{
public:	
	SplineIKControlModGeomPipelineEnumProc(SplineIKControlModData &smd) : mFound(false), mSplIKCtrlModData(smd) {}
	PipeEnumResult proc(ReferenceTarget *object, IDerivedObject *derObj, int index);
	bool mFound;
	SplineIKControlModData& mSplIKCtrlModData;

protected:
	SplineIKControlModGeomPipelineEnumProc(); // disallowed
	SplineIKControlModGeomPipelineEnumProc(SplineIKControlModGeomPipelineEnumProc& rhs); // disallowed
	SplineIKControlModGeomPipelineEnumProc& operator=(const SplineIKControlModGeomPipelineEnumProc& rhs); // disallowed
};

PipeEnumResult SplineIKControlModGeomPipelineEnumProc::proc(
	ReferenceTarget *object, 
	IDerivedObject *derObj, 
	int index)
{
	ModContext* curModContext = NULL;
	LocalModData* curLocalModData = NULL;
	if (derObj != NULL &&
		(curModContext = derObj->GetModContext(index)) != NULL &&
		(curLocalModData = curModContext->localData) == &mSplIKCtrlModData)
	{
...
INode* SplineIKControl::GetNodeFromModContext(SplineIKControlModData *smd, int &which)
{
	...
	for (i = 0; i < dep.Nodes.Count(); i++)
	{
		...
		if (node)
		{
			...
			SplineIKControlModGeomPipelineEnumProc pipeEnumProc(*smd);
