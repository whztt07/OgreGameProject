...
static ParamBlockDesc2 solidifypw_param_blk ( solidifypw_params, _T("params"),  0, &SolidifyPWDesc, 
	...
	pb_amount, 			_T("innerAmount"), 		TYPE_WORLD, 	P_ANIMATABLE, 	IDS_IAMOUNT, 
		...
		p_ui, 			TYPE_SPINNER,		EDITTYPE_UNIVERSE, IDC_EDIT,	IDC_SPIN, 0.01f, 
		end,

	pb_oamount, 			_T("outerAmount"), 		TYPE_WORLD, 	P_ANIMATABLE, 	IDS_OAMOUNT, 
		p_default, 		1.0f, 
		p_range, 		0.0f,999000.0f, 
		p_ui, 			TYPE_SPINNER,		EDITTYPE_UNIVERSE, IDC_EDIT2,	IDC_SPIN2, 0.01f, 
		end,
		...
		PartID& partID,  RefMessage message) 
{
	...
	if (message == REFMSG_CHANGE)
	{
		if (hTarget == pblock)
		{
			...
			{
				if ((partID == PART_TOPO) || (partID == PART_GEOM) || (partID == PART_ALL))
					return REF_SUCCEED;
...
INT_PTR SolidifyPWDlgProc::DlgProc(
