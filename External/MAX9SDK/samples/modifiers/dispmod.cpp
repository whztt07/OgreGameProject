...
public:
	...
	DispPathAccessor<T>(T* aDisp) : mDisp(aDisp) {}
...
template<class T>
MaxSDK::Util::Path  DispPathAccessor<T>::GetPath() const	{
	return MaxSDK::Util::Path(mDisp->bi.Name());
}

template<class T>
void DispPathAccessor<T>::SetPath(const MaxSDK::Util::Path& aNewPath)	{
	mDisp->bi.SetName(aNewPath.GetCStr());	
	mDisp->FreeBitmap();
	mDisp->LoadBitmap();
...
static void DisplacementMapNotify(void *param, NotifyInfo *info) 
{
	...
	case NOTIFY_BITMAP_CHANGED: 
		{
			TCHAR *l_fileName = (TCHAR *)info->callParam;
...
Point3 DisplaceField::Force(TimeValue t,const Point3 &pos, const Point3 &vel,int index)
	...
	if (dobj == NULL) return Point3(0.0f,0.0f,0.0f);  //667105 watje xrefs can change the base object type through proxies which will cause this to be null or the user can copy a new object type over ours
