...
*/
	...
	void Dump()
	{
		DebugPrint("Morph Name : %s\n",name);
		DebugPrint("Morph TM in parent Space:\n");
		...
		DebugPrint("Morph Angle : %f\n",influenceAngle);
		DebugPrint("Morph falloff : %d\n",falloff);
		DebugPrint("Morph wieght : %f\n",weight);
		DebugPrint("Morph flags : %d\n",flags);
		DebugPrint("Morph externalNodeID : %d\n",externalNodeID);
		DebugPrint("Morph externalFalloffID : %d\n",externalFalloffID);
		DebugPrint("\n");
		DebugPrint("Morph Count : %d\n",d.Count());
		for (int i = 0; i < d.Count(); i++)
		{
			DebugPrint("	ID %d\n",d[i].vertexID);
			DebugPrint("	localOwner %d\n",d[i].localOwner);
			Point3 v;
			v = d[i].vec;
			DebugPrint("	vec  %f %f %f\n",v.x,v.y,v.x);
			v = d[i].vecParentSpace;
			DebugPrint("	vecP %f %f %f\n",v.x,v.y,v.x);
			v = d[i].originalPt;
			DebugPrint("	Pt   %f %f %f\n",v.x,v.y,v.x);
		}
