...
//	TVITEM tvi;
	...
	//DebugPrint("Morph name creatiuon\n");
...
INT_PTR MorphByBoneParamsMapDlgProcSoftSelection::DlgProc(TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)		
	...
	switch (msg) 
		...
		case WM_INITDIALOG:
			...
			mod->pblock->GetValue(pb_selectiongraph,0,ref, FOREVER);
			mod->curveCtl = (ICurveCtl *) ref;


			mod->curveCtl->SetCustomParentWnd(GetDlgItem(hWnd, IDC_CURVE));
			mod->curveCtl->SetMessageSink(hWnd);
			mod->curveCtl->SetActive(TRUE);
			mod->pCurve = mod->curveCtl->GetControlCurve(0);
