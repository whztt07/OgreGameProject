...
/*
		if (ld->selfNode == NULL)
			DebugPrint("Adding ld %d:null\n",ld->id);
		else DebugPrint("Adding ld %d:%s\n",ld->id,ld->selfNode->GetName());
*/
...
//		if (i < morph->d.Count())
		{
			...
			if (process)
			{
				//see if we have an idex already
				int index = -1;
				...
				else initialP = localDataList[0]->originalPoints[i];
