...
public:
	void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval &valid)
	{
		...
		switch (id)
		{
			case pc_cache_file:
			{
				// TYPE_STRING's are null until you first assign a string to them
				if (v.s == NULL)
				{
					v.s = _T("");
				}
	...
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)    // set from v
	{
		...
		switch (id)
		{
			...
			/*	if (pointCache->m_pblock->GetInt(pc_force_unc_path, 0))
				{
					TCHAR uncName[MAX_PATH];
					if (BMMGetUniversalName(uncName,v.s))
					{
						pointCache->m_pblock->SetValue(pc_cache_file, 0, uncName);
					}
				}
			*/

				TCHAR* fname;
				pointCache->m_pblock->GetValue(pc_cache_file, 0, fname, FOREVER);
				pointCache->m_hCacheFile = pointCache->OpenCacheForRead(fname);
				if (pointCache->m_hCacheFile)
				{
...
void PointCacheParamsMapDlgProc::Update(TimeValue t)
{
	if (thishWnd)
	{
		//cache file label
	/*	TCHAR *fname;
		m->m_pblock->GetValue(pc_cache_file,0,fname,FOREVER);
		TSTR str(fname);
		TSTR path, file, ext;
		SplitFilename(str, &path, &file, &ext);
		TSTR name;
		if (file.isNull()) file = GetString(IDS_NONE);
		else
			name = file+ext;
		HWND hTextWnd = GetDlgItem(thishWnd,IDC_CACHEFILE_LABEL);
		SetWindowText(hTextWnd,file);

		ICustStatus *iStatus = GetICustStatus(GetDlgItem(thishWnd,IDC_FILENAME));
	    iStatus->SetText(name);
		MaxSDK::Util::Path fullpath(fname);
		IPathConfigMgr* pathMgr = IPathConfigMgr::GetPathConfigMgr();
		if (pathMgr->SearchForExternalFiles(fullpath)) 
		{
			iStatus->SetTooltip(TRUE,fullpath.GetString());
		}
		else
...
*/
		...
		if (m->TestError(ERROR_CACHEMISSING))
			str = GetString(IDS_ERROR_CACHEMISSING);
		else if (m->TestError(ERROR_CACHEINVALID))
			str = GetString(IDS_ERROR_CACHEINVALID);
		else if (m->TestError(ERROR_NUMPOINTS))
			str.printf(GetString(IDS_ERROR_NUMPOINTS), m->m_cacheNumPoints);
		else if (m->TestError(ERROR_POINTCOUNTCHANGED))
			str.printf(GetString(IDS_ERROR_POINTCOUNTCHANGED));
		else if (m->TestError(ERROR_NONODEFOUND))
			str.printf(GetString(IDS_ERROR_NONODEFOUND));
		else if(m->TestError(ERROR_NO_DEFORM_IN_FILE))
			str.printf(GetString(IDS_NO_DEFORM_IN_FILE));
		else {
			float cacheEndFrame = m->m_cacheStartFrame + ((m->m_cacheNumSamples-1) / (1.0f / m->m_cacheSampleRate));
			str.printf(GetString(IDS_CACHEINFO),	m->m_cacheNumPoints,
													m->m_cacheSampleRate,
													m->m_cacheNumSamples,
													m->m_cacheStartFrame,
													cacheEndFrame);
		}
		//str.printf("Error: %u",m->m_errorFlags);
		SetWindowText(GetDlgItem(thishWnd,IDC_MESSAGE_LABEL),str);
		...
		SetWindowText(GetDlgItem(thishWnd,IDC_CACHEINFO_GROUP),str);
	...
	if(m->m_fileType== PointCache::ePTS)
	{
		EnableWindow(GetDlgItem(thishWnd,IDC_RECORD),FALSE);
		EnableWindow(GetDlgItem(thishWnd,IDC_APPLY_TO_WHOLE_OBJECT),FALSE);
		EnableWindow(GetDlgItem(thishWnd,IDC_PLAYBACK_TYPE),FALSE);
		EnableWindow(GetDlgItem(thishWnd,IDC_LOAD_TYPE_SLAVE),FALSE);
		EnableWindow(GetDlgItem(thishWnd,IDC_LOAD_TYPE),FALSE);
		EnableWindow(GetDlgItem(thishWnd,IDC_NEW),FALSE);
	}
	else //pc2
	{
	
		EnableWindow(GetDlgItem(thishWnd,IDC_RECORD),TRUE);
		EnableWindow(GetDlgItem(thishWnd,IDC_APPLY_TO_WHOLE_OBJECT),TRUE);
		EnableWindow(GetDlgItem(thishWnd,IDC_PLAYBACK_TYPE),TRUE);
		EnableWindow(GetDlgItem(thishWnd,IDC_LOAD_TYPE_SLAVE),TRUE);
		EnableWindow(GetDlgItem(thishWnd,IDC_LOAD_TYPE),TRUE);
		EnableWindow(GetDlgItem(thishWnd,IDC_NEW),TRUE);
	}
}
	...
	switch (msg)
	{
		...
		case WM_COMMAND:
		{
			switch (LOWORD(wParam))
			{
				...
				case IDC_BROWSE:
				{
					TCHAR* str = m->m_pblock->GetStr(pc_cache_file);

					if (m->IsValidFName(str))
					{
						TSTR fullPath(str);
						TSTR path;
						SplitPathFile(fullPath, &path, NULL);
...
void PointCacheOSM::SetFileName(const TCHAR *filename)
{
...
void PointCacheWSM::SetFileName(const TCHAR *filename)
{
	if(dlgProc&&dlgProc->thishWnd)
	{
		//cache file label
...
void PointCache::SetCacheFile(const TCHAR *file)
{
	m_pblock->SetValue(pc_cache_file,0,const_cast<TCHAR *>(file));
	SetFileType(file);
}
...
void ReplaceOldPointCache(OLDPointCacheBase *obj)
{
	...
	for (int i = 0; i < dep.Nodes.Count(); i++)
	{
		...
		if (node)
		{
			...
			{
				TCHAR * filename = obj->GetCacheFile();
				if(filename&&_tcslen(filename)>0)
				{
					...
					if (pathMgr->SearchForExternalFiles(path)) 
					{
						...
						PC->SetCacheFile(path.GetString().data());
					}
				}
				...
				//been modified to handle this correclty in AddModifier. Defect 798844 MZ.
