...
CacheBuffer::CacheBuffer(const TSTR& filename)
{
	m_filename	= filename;
	m_refCount	= 0;
	m_buffer	= NULL;
...
{
	TSTR filename = pcMod->m_pblock->GetStr(pc_cache_file, 0);
	int cacheIndex = 0;
	if (GetCacheIndex(filename, cacheIndex))
	{
		...
		CacheBuffer* cacheBuffer = new CacheBuffer(filename);
...
bool PointCacheManager::GetCacheIndex(const TSTR& filename, int& index)
{
	int searchIndex = 0;
	while (searchIndex != m_cacheBuffers.Count())
	{
		if (m_cacheBuffers[searchIndex]->m_filename == filename) break;
		++searchIndex;
	}
