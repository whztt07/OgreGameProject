...
TCHAR * OLDPointCacheBase::GetCacheFile()
{
	TCHAR *f;
	pblock->GetValue(pb_cache_file,0,f,FOREVER);
	return f;
}
...
FILE* OLDPointCacheBase::OpenFileForRead(TCHAR *filename)
{
	...
	FILE *f =  fopen(path.GetCStr(), "rb");

	return f;
}

FILE* OLDPointCacheBase::OpenFileForWrite( TCHAR *filename)
{
	MaxSDK::Util::Path path(filename);
	IPathConfigMgr* pathMgr = IPathConfigMgr::GetPathConfigMgr();
	if (!pathMgr->SearchForExternalFiles(path)) 
		return NULL;
	FILE *f =  fopen(path.GetCStr(), "wb");
...
//		spobj->valid = FALSE;
	...
	outFile = OpenFileForWrite(fname);
...
int OLDPointCacheBase::PerformPTSMod(TCHAR *fname,float startTime,		BOOL rO,float s,TimeValue t, int &numPoints,float &sampleRate, int &numSamples, 
...
//DebugPrint("time %d  index %d   %f %f %f\n",t/160,i,p.x,p.y,p.z);
	...
	{ //temp disable
		...
		int error = PerformPTSMod(fname,startTime,relativeOffset,strength,t, numPoints,sampleRate,numSamples,mc,os,node);
...
MaxSDK::Util::Path OLDPointCacheAssetAccessor::GetPath() const	{
	TCHAR *fname = NULL;
	Interval iv;
...
void OLDPointCacheAssetAccessor::SetPath(const MaxSDK::Util::Path& aNewPath)	{
...
int OLDPointCacheAssetAccessor::GetAssetType() const	{
...
void OLDPointCacheBase::EnumAuxFiles(NameEnumCallback& nameEnum, DWORD flags) 
{
