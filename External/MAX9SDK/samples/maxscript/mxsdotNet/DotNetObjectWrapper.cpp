...
Value* DotNetObjectWrapper::intern(System::Object ^ object)
{
	try
	{
		...
		vl.result = CDotNetHostWnd::GetMXSContainerForObject(object);
		if (!vl.result)
			vl.result = DotNetObject::GetMXSContainerForObject(object);
		if (!vl.result)
		{
			dotNetObject *l_p_dotNetObject;
			vl.result = l_p_dotNetObject = new dotNetObject();
			l_p_dotNetObject->m_pDotNetObject = new DotNetObject(object, l_p_dotNetObject);
		}
...
Value* DotNetObjectWrapper::get_property(Value** arg_list, int count)
{
	...
	try
	{
		...
		for(int f=0; f< l_pFields->Length; f++)
		{
			...
			if (System::String::Compare(l_pPropString, l_pFieldName, true) == 0) 
			{
		...
		if (!found)
		{
			...
			for(int m=0; m < l_pProps->Length; m++)
			{
				...
				if (System::String::Compare(l_pPropString, l_pPropName, true) == 0) 
					break;
...
Value* DotNetObjectWrapper::set_property(Value** arg_list, int count)
{
	...
	try
	{
		...
		for(int f=0; f<fields->Length; f++)
		{
			...
			if (System::String::Compare(pPropString, pFieldName, true) == 0) 
			{
		...
		if (!found)
		{
			...
			for(int m=0; m<props->Length; m++)
			{
				...
				if (System::String::Compare(pPropString, pPropName, true) == 0) 
					break;
...
void DotNetObjectWrapper::RunEventHandlers(Value* target, Value* eventName, Array* handlers, array<System::Object^> ^delegateArgsArray)
{
	...
	for (int i = 0; i < handlers->size; i++)
	{
		...
		if (handler->tag == class_tag(MAXScriptFunction))
		{
			save_current_frames();
...
bool DotNetObjectWrapper::HasEvent(MCHAR *eventName)
{
	...
	for(int e=0; e < l_pEvents->Length; e++)
	{
		...
		if (System::String::Compare(l_pEventNameString,l_pEventName, true))
			return true;
	}
	return false;
}
