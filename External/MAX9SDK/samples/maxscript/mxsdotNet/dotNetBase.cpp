...
dotNetBase::get_property(Value** arg_list, int count)
{
	...
	if (GetDotNetObjectWrapper() && ( result = GetDotNetObjectWrapper()->get_property(arg_list, count)))
...
dotNetBase::set_property(Value** arg_list, int count)
{
	...
	Value* result;
	if (GetDotNetObjectWrapper() && (result = GetDotNetObjectWrapper()->set_property(arg_list, count)))
		return_protected(result)
	else
