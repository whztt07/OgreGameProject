...
// setVert <Mesh mesh> <vertlist> <point3 pos> node:<node=unsupplied>
Value*
meshop_setVert_cf(Value** arg_list, int count)
{
	...
	BitArray verts (nVerts);
	ValueToBitArrayM(arg_list[1], verts, nVerts, GetString(IDS_MESH_VERTEX_INDEX_OUT_OF_RANGE), MESH_VERTSEL_ALLOWED, mesh);
	Point3 pos =     arg_list[2]->to_point3(); // LOCAL COORDINATES!!!
	Value* inNode = key_arg(node);
...
//	else world_from_current_coordsys(pos);
	...
	for (int vertIndex = 0; vertIndex < nVerts; vertIndex++)
		if (verts[vertIndex]) tmd.Move (vertIndex, pos-mesh->verts[vertIndex]);
	if (mdu) 
		GetMeshDeltaUserDataInterface(owner)->ApplyMeshDelta (tmd, mdu, MAXScript_time());
	else 
		tmd.Apply (*mesh);
...
// moveVert <Mesh mesh> <vertlist> <point3 offset> node:<node=unsupplied>
Value*
meshop_moveVert_cf(Value** arg_list, int count)
{
	...
	Point3 pos =     arg_list[2]->to_point3(); // LOCAL COORDINATES!!!
	Value* inNode = key_arg(node);
	if (is_node(inNode)) ((MAXNode*)inNode)->object_from_current_coordsys_scaleRotate(pos);
	else if (is_node(arg_list[0])) ((MAXNode*)arg_list[0])->object_from_current_coordsys_scaleRotate(pos);
//	else world_from_current_coordsys(pos);
	MeshDeltaUser *mdu = (owner) ? GetMeshDeltaUserInterface(owner) : NULL;
	if (mdu && mdu->Editing()) mdu->ExitCommandModes();
	MeshDelta tmd (*mesh);
	for (int vertIndex = 0; vertIndex < nVerts; vertIndex++)
		if (verts[vertIndex]) tmd.Move (vertIndex, pos);
	if (mdu) 
		GetMeshDeltaUserDataInterface(owner)->ApplyMeshDelta (tmd, mdu, MAXScript_time());
	else 
		tmd.Apply (*mesh);
