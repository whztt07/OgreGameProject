...
/* 
// test block for seeing how to walk the reference tree
	if (owner && *owner) {
		RefList& theRefs = (*owner)->GetRefList();
		RefListItem* item;
	...
	if ((sc = obj->SuperClassID()) == GEN_DERIVOB_CLASS_ID)
	{
		...
		while (sc == GEN_DERIVOB_CLASS_ID)
		{
			for (int i = 0; i < dobj->NumModifiers(); i++)
			{
				...
				if (target_mod && mod == target_mod && (lmd = dobj->GetModContext(i)->localData))
					return GetMeshSelectDataInterface(lmd);
				else if ((lmd = dobj->GetModContext(i)->localData) &&
						 (emd = GetMeshSelectDataInterface(lmd)) &&
						  mod_index-- == 0)
				{
					target_mod = mod;
					return emd;
	...
	if ((sc = obj->SuperClassID()) == GEN_DERIVOB_CLASS_ID)
	{
		...
		while (sc == GEN_DERIVOB_CLASS_ID)
		{
			for (int i = 0; i < dobj->NumModifiers(); i++)
			{
				...
				else if ((mdu = GetMeshDeltaUserInterface(mod)) && mod_index-- == 0)
				{
					target_mod = mod;
					return mdu;
				}
