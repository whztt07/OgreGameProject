...
#include "app.h"
#include "resource.h"
#include "..\..\..\include\utilexp.h"
#include "..\..\..\include\utilintf.h"
...
#ifndef DESIGN_VER
#define REGISTRY_KEY "max Finder"
#else
#define REGISTRY_KEY "VIZ Finder"
#endif // !DESIGN_VER

	static TCHAR regKey[MAX_PATH];
	static bool init = true;
	if(init)	{
		_snprintf(regKey, MAX_PATH, "%s\\%s", UtilityInterface::GetRegistryKeyBase(), REGISTRY_KEY);
		init = false;
	}
	return regKey;
}
...
void App::SetRegSettings()
	...
	HKEY	hKey;
	DWORD	disp;
	if (RegCreateKeyEx(HKEY_CURRENT_USER, GetRegistryKey(), 0, "", REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &disp) == ERROR_SUCCESS) {
				...
				if (S_OK == hr) {
					for (int i=0; i<idx; i++) {
						if (pPropVar[i].vt == VT_LPSTR) {
							...
							int nSize = static_cast<int>(wcslen(pPropVar[i].pwszVal)+1);	// SR DCAST64: Downcast to 2G limit.
