...
INT_PTR CALLBACK PropDlgProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
	{
	App* pApp = reinterpret_cast<App*>(static_cast<LONG_PTR>(GetWindowLongPtr(hWnd, GWLP_USERDATA)));
		...
		if (S_OK == pUserDefinedPropertyStorage->Enum(&pIPropertyEnum)) {
			...
			if (S_OK == hr) {
				for (int i=0; i<idx; i++) {
					...
					item.iSubItem = 0;
					item.pszText = szBuf;
					item.cchTextMax = static_cast<int>(strlen(szBuf));	// SR DCAST64: Downcast to 2G limit.
...
#endif
      case VT_I8:
         sprintf(szString, "%I64d", pProp->hVal.QuadPart);
         break;
      case VT_INT:
         sprintf(szString, "%d", pProp->intVal);
         break;
		case VT_R4:
