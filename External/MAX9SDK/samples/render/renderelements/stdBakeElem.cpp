...
BaseInterface* DiffuseBakeElement::GetInterface(Interface_ID id) {
	...
	if(id == IMRSHADERTRANSLATION_INTERFACE_ID) {
		...
		return MaxBakeElement::GetInterface(id);
	}
}
//-----------------------------------------------------------------------------
...
public:
	...
	void *			Create(BOOL loading) { return new HeightBakeElement; }
...
public:
	...
	void *			Create(BOOL loading) { return new AmbientOcclusionBakeElement; }
...
void AmbientOcclusionBakeElement::SetParamFPValue1( int nParam, FPValue newVal ) {
	if( nParam > 0 && nParam <= GetNParams() ) {
		switch( nParam ) {
			case 1: mpParamBlk->SetValue( kParameterID_samples, 0, newVal.i );				break;
			case 2: mpParamBlk->SetValue( kParameterID_bright, 0, (AColor)*(newVal.p4) );	break;
			case 3: mpParamBlk->SetValue( kParameterID_dark, 0, (AColor)*(newVal.p4) );		break;
			case 4: mpParamBlk->SetValue( kParameterID_spread, 0, newVal.f );				break;
			case 5: mpParamBlk->SetValue( kParameterID_maxDistance, 0, newVal.f );			break;
			case 6: mpParamBlk->SetValue( kParameterID_falloff, 0, newVal.f );				break;
		}
	}
}
...
imrShader* AmbientOcclusionBakeElement::GetMRShader(imrShaderCreation& shaderCreation) {
	...
	if(genericElementShader != NULL) {
		...
		imrShader* ambientOcclusionShader = shaderCreation.CreateShader(_T("mib_amb_occlusion"), ElementName());
...
void AmbientOcclusionBakeElement::TranslateParameters(imrTranslation& translationInterface, imrShader* shader, TimeValue t, Interval& valid) {
	...
	// The parameters need to be passed to the ambient occlusin shader, which was already assigned as
			...
			if(GetParamIDByName(subShaderParamID, _T("subShader"), paramsPBlock)) {
				...
				if((ambientOcclusionTexmap != NULL) && IsIMRShader(ambientOcclusionTexmap)) {
					...
					if(ambientOcclusionPBlock != NULL) {
						...
						float maxDistance;
						float falloff;
						mpParamBlk->GetValue(kParameterID_samples, t, samples, valid);
						mpParamBlk->GetValue(kParameterID_bright, t, bright, valid);
						mpParamBlk->GetValue(kParameterID_dark, t, dark, valid);
						mpParamBlk->GetValue(kParameterID_spread, t, spread, valid);
						mpParamBlk->GetValue(kParameterID_maxDistance, t, maxDistance, valid);
						mpParamBlk->GetValue(kParameterID_falloff, t, falloff, valid);

						// Set the parameter values in the shader
						TranslateMRShaderParameter(ambientOcclusionPBlock, _T("samples"), samples);
						TranslateMRShaderParameter(ambientOcclusionPBlock, _T("bright"), bright);
						TranslateMRShaderParameter(ambientOcclusionPBlock, _T("dark"), dark);
						TranslateMRShaderParameter(ambientOcclusionPBlock, _T("spread"), spread);
						TranslateMRShaderParameter(ambientOcclusionPBlock, _T("max_distance"), maxDistance);
						TranslateMRShaderParameter(ambientOcclusionPBlock, _T("falloff"), falloff);
